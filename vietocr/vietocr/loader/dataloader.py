...
from vietocr.tool.translate import process_image, resize
...
class OCRDataset(Dataset):
    def __init__(
        ...
        self.env = lmdb.open(self.lmdb_path, max_readers=8, readonly=True, lock=False, readahead=False, meminit=False)
    ...
    def build_cluster_indices(self):
        ...
        pbar = tqdm(
            range(self.__len__()), desc="{} build cluster".format(self.lmdb_path), ncols=100, position=0, leave=True
        )
...
class ClusterRandomSampler(Sampler):
    ...
    def __iter__(self):
        ...
        for cluster, cluster_indices in self.data_source.cluster_indices.items():
            ...
            batches = [
                cluster_indices[i : i + self.batch_size] for i in range(0, len(cluster_indices), self.batch_size)
            ]
...
class Collator(object):
    ...
    def __call__(self, batch):
        ...
        for sample in batch:
            ...
            tgt = np.concatenate((label, np.zeros(max_label_len - label_len, dtype=np.int32)))
            ...
            target_weights.append(
                np.concatenate(
                    (np.ones(one_mask_len, dtype=np.float32), np.zeros(max_label_len - one_mask_len, dtype=np.float32))
                )
            )

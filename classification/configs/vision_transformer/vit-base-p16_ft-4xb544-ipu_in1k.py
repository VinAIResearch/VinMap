...
model = dict(
    ...
    backbone=dict(
        img_size=224, init_cfg=dict(type="Pretrained", checkpoint=pretrained, _delete_=True, prefix="backbone")
    ),
)
...
runner = dict(type="IterBasedRunner", ipu_model_cfg=ipu_model_cfg, options_cfg=options_cfg, max_iters=5000)

...
policies = [
    [dict(type="Posterize", bits=4, prob=0.4), dict(type="Rotate", angle=30.0, prob=0.6)],
    ...
    [dict(type="Posterize", bits=6, prob=0.8), dict(type="Equalize", prob=1.0)],
    [dict(type="Rotate", angle=10.0, prob=0.2), dict(type="Solarize", thr=256 / 9, prob=0.6)],
    [dict(type="Equalize", prob=0.6), dict(type="Posterize", bits=5, prob=0.4)],
    [dict(type="Rotate", angle=30 / 9 * 8, prob=0.8), dict(type="ColorTransform", magnitude=0.0, prob=0.4)],
    [dict(type="Rotate", angle=30.0, prob=0.4), dict(type="Equalize", prob=0.6)],
    [dict(type="Equalize", prob=0.0), dict(type="Equalize", prob=0.8)],
    [dict(type="Invert", prob=0.6), dict(type="Equalize", prob=1.0)],
    [dict(type="ColorTransform", magnitude=0.4, prob=0.6), dict(type="Contrast", magnitude=0.8, prob=1.0)],
    [dict(type="Rotate", angle=30 / 9 * 8, prob=0.8), dict(type="ColorTransform", magnitude=0.2, prob=1.0)],
    ...
    [dict(type="Shear", magnitude=0.3 / 9 * 5, prob=0.6, direction="horizontal"), dict(type="Equalize", prob=1.0)],
    [dict(type="ColorTransform", magnitude=0.0, prob=0.4), dict(type="Equalize", prob=0.6)],
    [dict(type="Equalize", prob=0.4), dict(type="Solarize", thr=256 / 9 * 5, prob=0.2)],
    [dict(type="Solarize", thr=256 / 9 * 4, prob=0.6), dict(type="AutoContrast", prob=0.6)],
    [dict(type="Invert", prob=0.6), dict(type="Equalize", prob=1.0)],
    [dict(type="ColorTransform", magnitude=0.4, prob=0.6), dict(type="Contrast", magnitude=0.8, prob=1.0)],
...
data = dict(samples_per_gpu=128, workers_per_gpu=4, train=dict(pipeline=train_pipeline))
evaluation = dict(interval=10, metric="accuracy")

# optimizer
optimizer = dict(type="RMSprop", lr=0.064, alpha=0.9, momentum=0.9, eps=0.0316, weight_decay=1e-5)

...
def plot_phase_train(metric, log_dict, epochs, curve_label, json_log):
    ...
    if metric not in log_dict[epochs[0]]:
        raise KeyError(f"{json_log} does not contain metric {metric}" f" in train mode")
    xs, ys = [], []
    for epoch in epochs:
        ...
        assert len(iters) > 0, "The training log is empty, please try to reduce the " "interval of log in config file."
    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        for j, metric in enumerate(metrics):
            ...
            if any(m in metric for m in TEST_METRICS):
                ...
                plot_phase_train(metric, log_dict, epochs, curve_label, json_log)
            plt.legend()
...
def add_plot_parser(subparsers):
    ...
    parser_plt.add_argument("--style", type=str, default="whitegrid", help="style of plt")

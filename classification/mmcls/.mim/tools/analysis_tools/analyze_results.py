...
def parse_args():
    ...
    parser.add_argument("--topk", default=20, type=int, help="Number of images to select for success/fail")
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
...
def main():
    ...
    assert "pred_score" in outputs and "pred_class" in outputs and "pred_label" in outputs, (
        'No "pred_label", "pred_score" or "pred_class" in result file, ' 'please set "--out-items" in test.py'
    )
    ...
    for info in dataset.data_infos:
        if info["img_prefix"] is not None:
            filename = osp.join(info["img_prefix"], info["img_info"]["filename"])
        else:
    ...
    need_keys = ["filename", "gt_label", "gt_class", "pred_score", "pred_label", "pred_class"]

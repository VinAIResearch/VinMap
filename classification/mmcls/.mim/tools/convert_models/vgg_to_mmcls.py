...
def get_layer_maps(layer_num, with_bn):
    ...
    if with_bn:
        if layer_num == 11:
            ...
            layer_idxs = [0, 3, 7, 10, 14, 17, 20, 23, 27, 30, 33, 36, 40, 43, 46, 49]
        else:
        ...
        if layer_num == 11:
            ...
            layer_idxs = [0, 2, 5, 7, 10, 12, 14, 16, 19, 21, 23, 25, 28, 30, 32, 34]
            new_layer_idxs = [0, 1, 3, 4, 6, 7, 8, 9, 11, 12, 13, 14, 16, 17, 18, 19]
        else:
            raise ValueError(f"Invalid number of layers: {layer_num}")

        layer_maps["conv"] = {layer_idx: new_layer_idx for layer_idx, new_layer_idx in zip(layer_idxs, new_layer_idxs)}
        ...
        if "features" in key:
            ...
            for layer_key, maps in layer_maps.items():
                if layer_idx in maps:
                    ...
                    new_key = delimiter.join([prefix, "features", str(new_layer_idx), layer_key, weight_type])
...
def main():
    ...
    parser.add_argument("--bn", action="store_true", help="whether original vgg has BN")
    parser.add_argument("--layer-num", type=int, choices=[11, 13, 16, 19], default=11, help="number of VGG layers")

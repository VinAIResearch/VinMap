...
def parse_args():
    parser = argparse.ArgumentParser(description="Test (and eval) an ONNX model using ONNXRuntime.")
...
def main():
    ...
    if args.backend == "onnxruntime":
        model = ONNXRuntimeClassifier(args.model, class_names=dataset.CLASSES, device_id=0)
    elif args.backend == "tensorrt":
        model = TensorRTClassifier(args.model, class_names=dataset.CLASSES, device_id=0)
    else:
    ...
    if args.metrics:
        ...
        results = {"pred_score": pred_score, "pred_label": pred_label, "pred_class": pred_class}
        if not args.out:
            print(

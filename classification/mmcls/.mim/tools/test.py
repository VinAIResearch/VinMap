...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        help="(Deprecated, please use --gpu-id) ids of gpus to use " "(only applicable to non-distributed testing)",
    )
    parser.add_argument(
...
def main():
    ...
    if "CLASSES" in checkpoint.get("meta", {}):
        ...
        warnings.warn("Class names are not saved in the checkpoint's " "meta data, use imagenet by default.")
    ...
    if not distributed:
        ...
        outputs = single_gpu_test(model, data_loader, args.show, args.show_dir, **show_kwargs)
    else:
        model = wrap_distributed_model(model, device=cfg.device, broadcast_buffers=False)
    ...
    if rank == 0:
        ...
        if args.metrics:
            eval_results = dataset.evaluate(
                results=outputs, metric=args.metrics, metric_options=args.metric_options, logger=logger
            )

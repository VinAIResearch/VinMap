...
from mmcv.runner import EpochBasedRunner, IterBasedRunner, IterLoader, build_optimizer
...
def parse_args():
    ...
    parser.add_argument("--ngpus", type=int, default=1, help="The number of GPUs used in training.")
    parser.add_argument("--title", type=str, help="title of figure")
    parser.add_argument("--style", type=str, default="whitegrid", help="style of plt")
    parser.add_argument("--save-path", type=Path, help="The learning rate curve plot save path")
    parser.add_argument(
...
def plot_curve(lr_list, args, iters_per_epoch, by_epoch=True):
    ...
    try:
        ...
        print(
            "Attention: The plot style won't be applied because 'seaborn' "
    ...
    if by_epoch:
        ...
        sec_ax = ax.secondary_xaxis("bottom", functions=(lambda x: x / iters_per_epoch, lambda y: y * iters_per_epoch))
...
def simulate_train(data_loader, cfg, by_epoch=True):
    ...
    if by_epoch:
        runner = DummyEpochBasedRunner(
            max_epochs=cfg.runner.max_epochs, model=model, optimizer=optimizer, logger=logger
        )
    else:
        runner = DummyIterBasedRunner(max_iters=cfg.runner.max_iters, model=model, optimizer=optimizer, logger=logger)
...
def main():
    ...
    if args.save_path and not args.save_path.parent.exists():
        raise Exception(
            f"The save path is {args.save_path}, and directory " f"'{args.save_path.parent}' do not exist."
        )

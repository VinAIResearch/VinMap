...
try:
    from pytorch_grad_cam import EigenCAM, EigenGradCAM, GradCAM, GradCAMPlusPlus, LayerCAM, XGradCAM
    from pytorch_grad_cam.activations_and_gradients import ActivationsAndGradients
    from pytorch_grad_cam.utils.image import show_cam_on_image
except ImportError:
    raise ImportError('Please run `pip install "grad-cam>=1.3.6"` to install ' "3rd party package pytorch_grad_cam.")
...
def parse_args():
    ...
    parser.add_argument(
        "--method", default="GradCAM", help="Type of method to use, supports " f'{", ".join(list(METHOD_MAP.keys()))}.'
    )
    parser.add_argument(
        "--target-category",
        default=[],
        nargs="+",
        type=int,
        help="The target category to get CAM, default to use result " "get from given model.",
    )
    parser.add_argument(
        "--eigen-smooth",
        default=False,
    ...
    parser.add_argument("--save-path", type=Path, help="The path to save visualize cam image, default not to save.")
    parser.add_argument("--device", default="cpu", help="Device to use cpu")
    parser.add_argument("--vit-like", action="store_true", help="Whether the network is a ViT-like network.")
    parser.add_argument(
        "--num-extra-tokens",
        type=int,
        help="The number of extra tokens in ViT-like backbones. Defaults to" " use num_extra_tokens of the backbone.",
    )
    parser.add_argument(
        "--cfg-options",
        nargs="+",
    ...
    if args.method.lower() not in METHOD_MAP.keys():
        raise ValueError(f"invalid CAM type {args.method}," f' supports {", ".join(list(METHOD_MAP.keys()))}.')
...
def build_reshape_transform(model, args):
    ...
    def _reshape_transform(tensor):
        ...
        assert len(tensor.size()) == 3, (
            f"The input feature's shape is {tensor.size()}, " "and the feature seems not from a vit-like network?"
        )
...
def init_cam(method, model, target_layers, use_cuda, reshape_transform):
    ...
    cam.activations_and_grads = MMActivationsAndGradients(cam.model, cam.target_layers, reshape_transform)
...
def get_layer(layer_str, model):
    ...
    def get_children_by_name(model, name):
        try:
            ...
            raise AttributeError(e.args[0] + ". Please use `--preview-model` to check keys at first.")
...
def get_default_traget_layers(model, args):
    ...
    if args.vit_like:
        if args.num_extra_tokens:
            ...
            raise AttributeError("Please set num_extra_tokens in backbone" " or using 'num-extra-tokens'")
        ...
        if num_extra_tokens >= 1:
            print("Automatically choose the last norm layer before the " "final attention block as target_layer..")

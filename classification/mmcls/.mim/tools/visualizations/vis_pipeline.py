...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir", default="", type=str, help="folder to save output pictures, if not set, do not save."
    )
    parser.add_argument(
        "--phase",
        default="train",
        ...
        help='phase of dataset to visualize, accept "train" "test" and "val".' " Default train.",
    )
    parser.add_argument(
        "--number",
        type=int,
    ...
    parser.add_argument(
        "--show", default=False, action="store_true", help="whether to display images in pop-up window. Default False."
    )
    parser.add_argument(
        "--adaptive",
        default=False,
    ...
    parser.add_argument(
        ...
        default=200,
        type=int,
        help="the min edge length when visualizing images, used when " '"--adaptive" is true. Default 200.',
    )
    parser.add_argument(
        "--max-edge-length",
        default=800,
        type=int,
        help="the max edge length when visualizing images, used when " '"--adaptive" is true. Default 1000.',
    )
    parser.add_argument("--bgr2rgb", default=False, action="store_true", help="flip the color channel order of images")
    parser.add_argument(
        "--window-size", default="12*7", help='size of the window to display images, in format of "$W*$H".'
    )
    parser.add_argument(
        "--cfg-options",
        nargs="+",
    ...
    if args.window_size != "":
        assert re.match(r"\d+\*\d+", args.window_size), "'window-size' must be in format 'W*H'."
    if args.output_dir == "" and not args.show:
        raise ValueError(
            "if '--output-dir' and '--show' are not set, " "nothing will happen when the program running."
        )
...
def retrieve_data_cfg(config_path, skip_type, cfg_options, phase):
    ...
    data_cfg["pipeline"] = [x for x in data_cfg.pipeline if x["type"] not in skip_type]
...
def build_dataset_pipelines(cfg, phase):
    ...
    data_cfg = cfg.data[phase]
    loadimage_pipeline = []
    if len(data_cfg.pipeline) != 0 and data_cfg.pipeline[0]["type"] == "LoadImageFromFile":
    ...
    pipelines = {pipeline_cfg["type"]: build_from_cfg(pipeline_cfg, PIPELINES) for pipeline_cfg in origin_pipeline}
...
def prepare_imgs(args, imgs: List[np.ndarray], steps=None):
    ...
    if args.adaptive:
        for i, img in enumerate(imgs):
            imgs[i] = adaptive_size(img, args.min_edge_length, args.max_edge_length)
    else:
        # if src image is too large or too small,
        # warning a "--adaptive" message.
        for ori_h, ori_w, _ in ori_shapes:
            if (
                args.min_edge_length > ori_h
                or args.min_edge_length > ori_w
...
def concat_imgs(imgs, steps, ori_shapes):
    ...
    for i, img in enumerate(imgs):
        ...
        img = cv2.copyMakeBorder(
            img, pad_top, pad_bottom, pad_left, pad_right, cv2.BORDER_CONSTANT, value=(255, 255, 255)
        )
...
def adaptive_size(image, min_edge_length, max_edge_length, src_shape=None):
    ...
    if image_h < min_edge_length or image_w < min_edge_length:
        image = mmcv.imrescale(image, min(min_edge_length / image_h, min_edge_length / image_h))
    if image_h > max_edge_length or image_w > max_edge_length:
        image = mmcv.imrescale(image, max(max_edge_length / image_h, max_edge_length / image_w))
    return image
...
def get_display_img(args, item, pipelines):
    ...
    if args.mode == "original":
        ...
        image = prepare_imgs(args, [pipeline_images[0], pipeline_images[-1]], steps)
    ...
    with vis.ImshowInfosContextManager(fig_size=(wind_w, wind_h)) as manager:
        for i, item in enumerate(itertools.islice(dataset, display_number)):
            ...
            ret, _ = manager.put_img_infos(
                image, infos, font_size=20, out_file=dist_path, show=args.show, **args.show_options
            )

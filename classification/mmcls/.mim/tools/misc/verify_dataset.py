...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="output path of all the broken files. If the specified path " "already exists, delete the previous file ",
    )
    parser.add_argument(
        "--phase",
        default="train",
    ...
    parser.add_argument("--num-process", type=int, default=1, help="number of process to use")
    parser.add_argument(
        "--cfg-options",
        nargs="+",
        action=DictAction,
...
class DatasetValidator:
    ...
    def __init__(self, dataset_cfg, log_file_path, phase):
        ...
        assert dataset_cfg.data[phase].pipeline[0]["type"] == "LoadImageFromFile", (
            "This tool is only for dataset " "that needs to load image from files."
        )
    ...
    def valid_idx(self, idx):
        ...
        try:
            ...
            with open(self.log_file_path, "a") as f:
                ...
                filepath = os.path.join(item["img_prefix"], item["img_info"]["filename"])
                f.write(filepath + "\n")
...
def print_info(log_file_path):
    ...
    with open(log_file_path, "r") as f:
        ...
        if context == "":
            ...
            print(f"{num_file} broken files found, name list save in file:" f"{log_file_path}")
    print()
...
def main():
    ...
    if args.num_process > 1:
        ...
        track_parallel_progress(
            validator.valid_idx, list(range(len(validator))), args.num_process, chunksize=chunksize, keep_order=False
        )
    else:

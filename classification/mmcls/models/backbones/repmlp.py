...
def fuse_bn(conv_or_fc, bn):
    ...
    if len(tmp_weight) == conv_or_fc.weight.size(0):
        return (conv_or_fc.weight * tmp_weight, bn.bias - bn.running_mean * bn.weight / std)
    else:
...
class RepMLPBlock(BaseModule):
    ...
    def __init__(
        ...
        self.gp = GlobalPerceptron(input_channels=channels, ratio=globalperceptron_ratio)
    ...
    def partition(self, x, h_parts, w_parts):
        ...
        x = x.reshape(-1, self.channels, h_parts, self.path_h, w_parts, self.path_w)
    ...
    def partition_affine(self, x, h_parts, w_parts):
        ...
        out = self.fc3_bn(out)
        out = out.reshape(-1, h_parts, w_parts, self.num_sharesets, self.path_h, self.path_w)
        return out
    ...
    def forward(self, inputs):
        ...
        if self.reparam_conv_kernels is not None and not self.deploy:
            conv_inputs = partitions.reshape(-1, self.num_sharesets, self.path_h, self.path_w)
            conv_out = 0
            for k in self.reparam_conv_kernels:
                conv_branch = self.__getattr__("repconv{}".format(k))
                conv_out += conv_branch(conv_inputs)
            conv_out = conv_out.reshape(-1, h_parts, w_parts, self.num_sharesets, self.path_h, self.path_w)
            fc3_out += conv_out
    ...
    def _convert_conv_to_fc(self, conv_kernel, conv_bias):
        ...
        in_channels = (
            torch.eye(self.path_h * self.path_w)
            .repeat(1, self.num_sharesets)
            .reshape(self.path_h * self.path_w, self.num_sharesets, self.path_h, self.path_w)
            .to(conv_kernel.device)
        )
        fc_k = F.conv2d(
            in_channels,
            conv_kernel,
        ...
        fc_k = fc_k.reshape(self.path_w * self.path_w, self.num_sharesets * self.path_h * self.path_w).t()
...
class RepMLPNet(BaseModule):
    ...
    arch_zoo = {
        **dict.fromkeys(
            ["b", "base"],
            {"channels": [96, 192, 384, 768], "depths": [2, 2, 12, 2], "sharesets_nums": [1, 4, 32, 128]},
        ),
    }  # yapf: disable
    ...
    def __init__(
        ...
        self.patch_hs = [self.patch_resolution[0] // 2**i for i in range(self.num_stage)]
        self.patch_ws = [self.patch_resolution[1] // 2**i for i in range(self.num_stage)]
        ...
        for stage_idx in range(self.num_stage):
            ...
            stage_blocks = [RepMLPNetUnit(**_stage_cfg) for _ in range(self.depths[stage_idx])]
    ...
    def forward(self, x):
        assert x.shape[2:] == self.img_size, (
            "The Rep-MLP doesn't support dynamic input shape. " f"Please input images with shape {self.img_size}"
        )

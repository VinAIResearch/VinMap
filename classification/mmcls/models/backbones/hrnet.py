...
class HRNet(BaseModule):
    ...
    def __init__(
        ...
        self.conv1 = build_conv_layer(
            self.conv_cfg, in_channels, out_channels=64, kernel_size=3, stride=2, padding=1, bias=False
        )
        ...
        self.conv2 = build_conv_layer(
            self.conv_cfg, in_channels=64, out_channels=64, kernel_size=3, stride=2, padding=1, bias=False
        )
        ...
        num_channels = [channel * get_expansion(block) for channel in base_channels]
        # To align with the original code, use layer1 instead of stage1 here.
        self.layer1 = ResLayer(block, in_channels=64, out_channels=num_channels[0], num_blocks=num_blocks[0])
        ...
        for i in range(2, 5):
            ...
            transition = self._make_transition_layer(pre_num_channels, num_channels)
            self.add_module(f"transition{i-1}", transition)
            stage = self._make_stage(stage_cfg, num_channels, multiscale_output=multiscale_output_)

...
class GlobalLocalFilter(nn.Module):
    ...
    def __init__(self, dim, h=14, w=8):
        ...
        self.pre_norm = HorNetLayerNorm(dim, eps=1e-6, data_format="channels_first")
        self.post_norm = HorNetLayerNorm(dim, eps=1e-6, data_format="channels_first")
...
class HorNetBlock(nn.Module):
    ...
    def __init__(
        self, dim, order=5, dw_cfg=dict(type="DW", kernel_size=7), scale=1.0, drop_path_rate=0.0, use_layer_scale=True
    ):
        super().__init__()
        self.out_channels = dim

        self.norm1 = HorNetLayerNorm(dim, eps=1e-6, data_format="channels_first")
...
class HorNet(BaseBackbone):
    ...
    def __init__(
        ...
        if fft is None:
            raise RuntimeError('Failed to import torch.fft. Please install "torch>=1.7".')
        ...
        for i in range(3):
            downsample_layer = nn.Sequential(
                HorNetLayerNorm(dims[i], eps=1e-6, data_format="channels_first"),
        ...
        norm_layer = partial(HorNetLayerNorm, eps=1e-6, data_format="channels_first")

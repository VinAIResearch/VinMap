...
class RepVGGBlock(BaseModule):
    ...
    def create_conv_bn(self, kernel_size, dilation=1, padding=0):
        ...
        conv_bn.add_module("norm", build_norm_layer(self.norm_cfg, num_features=self.out_channels)[1])
    ...
    def forward(self, x):
        ...
        def _inner_forward(inputs):
            ...
            inner_out = self.branch_3x3(inputs) + self.branch_1x1(inputs) + branch_norm_out
    ...
    def switch_to_deploy(self):
        ...
        if self.deploy:
            return
        assert self.norm_cfg["type"] == "BN", "Switch is not allowed when norm_cfg['type'] != 'BN'."
    ...
    def reparameterize(self):
        ...
        return (weight_3x3 + weight_1x1 + weight_norm, bias_3x3 + bias_1x1 + bias_norm)
    ...
    def _norm_to_conv3x3(self, branch_norm):
        ...
        conv_weight = torch.zeros((self.in_channels, input_dim, 3, 3), dtype=branch_norm.weight.dtype)
...
class RepVGG(BaseBackbone):
    ...
    arch_settings = {
        "A0": dict(num_blocks=[2, 4, 14, 1], width_factor=[0.75, 0.75, 0.75, 2.5], group_layer_map=None, se_cfg=None),
        "A1": dict(num_blocks=[2, 4, 14, 1], width_factor=[1, 1, 1, 2.5], group_layer_map=None, se_cfg=None),
        "A2": dict(num_blocks=[2, 4, 14, 1], width_factor=[1.5, 1.5, 1.5, 2.75], group_layer_map=None, se_cfg=None),
        "B0": dict(
            num_blocks=[4, 6, 16, 1], width_factor=[1, 1, 1, 2.5], group_layer_map=None, se_cfg=None, stem_channels=64
        ),
        "B1": dict(num_blocks=[4, 6, 16, 1], width_factor=[2, 2, 2, 4], group_layer_map=None, se_cfg=None),
        "B1g2": dict(num_blocks=[4, 6, 16, 1], width_factor=[2, 2, 2, 4], group_layer_map=g2_layer_map, se_cfg=None),
        "B1g4": dict(num_blocks=[4, 6, 16, 1], width_factor=[2, 2, 2, 4], group_layer_map=g4_layer_map, se_cfg=None),
        "B2": dict(num_blocks=[4, 6, 16, 1], width_factor=[2.5, 2.5, 2.5, 5], group_layer_map=None, se_cfg=None),
        "B2g2": dict(
            num_blocks=[4, 6, 16, 1], width_factor=[2.5, 2.5, 2.5, 5], group_layer_map=g2_layer_map, se_cfg=None
        ),
        "B2g4": dict(
            num_blocks=[4, 6, 16, 1], width_factor=[2.5, 2.5, 2.5, 5], group_layer_map=g4_layer_map, se_cfg=None
        ),
        "B3": dict(num_blocks=[4, 6, 16, 1], width_factor=[3, 3, 3, 5], group_layer_map=None, se_cfg=None),
        "B3g2": dict(num_blocks=[4, 6, 16, 1], width_factor=[3, 3, 3, 5], group_layer_map=g2_layer_map, se_cfg=None),
        "B3g4": dict(num_blocks=[4, 6, 16, 1], width_factor=[3, 3, 3, 5], group_layer_map=g4_layer_map, se_cfg=None),
        "D2se": dict(
        ...
        "yolox-pai-small": dict(
            num_blocks=[3, 5, 7, 3], width_factor=[1, 1, 1, 1], group_layer_map=None, se_cfg=None, stem_channels=32
        ),
    }
    ...
    def __init__(
        ...
        assert len(arch["num_blocks"]) == len(arch["width_factor"]) == len(strides) == len(dilations)
        assert max(out_indices) < len(arch["num_blocks"])
        if arch["group_layer_map"] is not None:
            assert max(arch["group_layer_map"].keys()) <= sum(arch["num_blocks"])
        ...
        channels = min(arch.get("stem_channels", 64), int(self.base_channels * self.arch["width_factor"][0]))
        ...
        for i in range(len(arch["num_blocks"])):
            ...
            out_channels = int(self.base_channels * 2**i * self.arch["width_factor"][i])

            stage, next_create_block_idx = self._make_stage(
                channels, out_channels, num_blocks, stride, dilation, next_create_block_idx, init_cfg
            )
        ...
        if add_ppf:
            self.ppf = MTSPPF(out_channels, out_channels, norm_cfg=norm_cfg, act_cfg=act_cfg, kernel_size=5)
        else:
            self.ppf = None

    def _make_stage(self, in_channels, out_channels, num_blocks, stride, dilation, next_create_block_idx, init_cfg):
        ...
        for i in range(num_blocks):
            groups = (
                self.arch["group_layer_map"].get(next_create_block_idx, 1)
                if self.arch["group_layer_map"] is not None
                else 1
            )
            blocks.append(
                RepVGGBlock(

...
class SwinBlockV2(BaseModule):
    ...
    def __init__(
        ...
        _attn_cfgs.update(
            window_msa=WindowMSAV2, msa_cfg=dict(pretrained_window_size=to_2tuple(pretrained_window_size))
        )
...
class SwinBlockV2Sequence(BaseModule):
    ...
    def __init__(
        ...
        for i in range(depth):
            extra_norm = True if extra_norm_every_n_blocks and (i + 1) % extra_norm_every_n_blocks == 0 else False
            _block_cfg = {
...
class SwinTransformerV2(BaseBackbone):
    ...
    arch_zoo = {
        **dict.fromkeys(
            ["t", "tiny"],
            {"embed_dims": 96, "depths": [2, 2, 6, 2], "num_heads": [3, 6, 12, 24], "extra_norm_every_n_blocks": 0},
        ),
        **dict.fromkeys(
            ["s", "small"],
            {"embed_dims": 96, "depths": [2, 2, 18, 2], "num_heads": [3, 6, 12, 24], "extra_norm_every_n_blocks": 0},
        ),
        **dict.fromkeys(
            ["b", "base"],
            {"embed_dims": 128, "depths": [2, 2, 18, 2], "num_heads": [4, 8, 16, 32], "extra_norm_every_n_blocks": 0},
        ),
        **dict.fromkeys(
            ["l", "large"],
            {"embed_dims": 192, "depths": [2, 2, 18, 2], "num_heads": [6, 12, 24, 48], "extra_norm_every_n_blocks": 0},
        ),
        # head count not certain for huge, and is employed for another
        # parallel study about self-supervised learning.
        **dict.fromkeys(
            ["h", "huge"],
            {"embed_dims": 352, "depths": [2, 2, 18, 2], "num_heads": [8, 16, 32, 64], "extra_norm_every_n_blocks": 6},
        ),
        **dict.fromkeys(
            ["g", "giant"],
            {
    ...
    def __init__(
        ...
        if isinstance(window_size, int):
            ...
            assert len(window_size) == self.num_layers, (
                f"Length of window_sizes {len(window_size)} is not equal to " f"length of stages {self.num_layers}."
            )

...
class SeesawLoss(nn.Module):
    ...
    def __init__(self, use_sigmoid=False, p=0.8, q=2.0, num_classes=1000, eps=1e-2, reduction="mean", loss_weight=1.0):
    ...
    def forward(self, cls_score, labels, weight=None, avg_factor=None, reduction_override=None):
        ...
        assert cls_score.size(0) == labels.view(-1).size(0), (
            f"Expected `labels` shape [{cls_score.size(0)}], " f"but got {list(labels.size())}"
        )

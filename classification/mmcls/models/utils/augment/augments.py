...
class Augments(object):
    ...
    def __init__(self, augments_cfg):
        ...
        if has_identity:
            assert (
                sum(self.augment_probs) == 1.0
            ), "The sum of augmentation probabilities should equal to 1," " but got {:.2f}".format(
                sum(self.augment_probs)
            )
        else:
            assert (
                sum(self.augment_probs) <= 1.0
            ), "The sum of augmentation probabilities should less than or " "equal to 1, but got {:.2f}".format(
                sum(self.augment_probs)
            )
            identity_prob = 1 - sum(self.augment_probs)
            if identity_prob > 0:
                num_classes = self.augments[0].num_classes
                self.augments += [build_augment(dict(type="Identity", num_classes=num_classes, prob=identity_prob))]

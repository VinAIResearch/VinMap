...
class RandAugment(object):
    ...
    def __init__(
        self, policies, num_policies, magnitude_level, magnitude_std=0.0, total_level=30, hparams=_HPARAMS_DEFAULT
    ):
        assert isinstance(num_policies, int), (
            "Number of policies must be " f"of int type, got {type(num_policies)} instead."
        )
        assert isinstance(magnitude_level, (int, float)), (
            "Magnitude level must be of int or float type, " f"got {type(magnitude_level)} instead."
        )
        assert isinstance(total_level, (int, float)), (
            "Total level must be " f"of int or float type, got {type(total_level)} instead."
        )
        assert isinstance(policies, list) and len(policies) > 0, "Policies must be a non-empty list."

        assert isinstance(magnitude_std, (Number, str)), (
            "Magnitude std must be of number or str type, " f"got {type(magnitude_std)} instead."
        )
        if isinstance(magnitude_std, str):
            assert magnitude_std == "inf", (
                'Magnitude std must be of number or "inf", ' f'got "{magnitude_std}" instead.'
            )
    ...
    def _check_policies(self, policies):
        for policy in policies:
            assert isinstance(policy, dict) and "type" in policy, 'Each policy must be a dict with key "type".'
            type_name = policy["type"]

            magnitude_key = policy.get("magnitude_key", None)
            if magnitude_key is not None:
                assert "magnitude_range" in policy, f"RandAugment policy {type_name} needs `magnitude_range`."
                magnitude_range = policy["magnitude_range"]
                assert isinstance(magnitude_range, Sequence) and len(magnitude_range) == 2, (
                    f"`magnitude_range` of RandAugment policy {type_name} " f"should be a Sequence with two numbers."
                )
...
class Shear(object):
    ...
    def __init__(
        ...
        assert isinstance(magnitude, (int, float)), (
            "The magnitude type must " f"be int or float, but got {type(magnitude)} instead."
        )
        if isinstance(pad_val, int):
            pad_val = tuple([pad_val] * 3)
        elif isinstance(pad_val, Sequence):
            assert len(pad_val) == 3, "pad_val as a tuple must have 3 " f"elements, got {len(pad_val)} instead."
            assert all(isinstance(i, int) for i in pad_val), "pad_val as a " "tuple must got elements of int type."
        else:
            raise TypeError("pad_val must be int or tuple with 3 elements.")
        assert 0 <= prob <= 1.0, "The prob should be in range [0,1], " f"got {prob} instead."
        assert direction in ("horizontal", "vertical"), (
            "direction must be " f'either "horizontal" or "vertical", got {direction} instead.'
        )
        assert 0 <= random_negative_prob <= 1.0, (
            "The random_negative_prob " f"should be in range [0,1], got {random_negative_prob} instead."
        )
    ...
    def __call__(self, results):
        ...
        for key in results.get("img_fields", ["img"]):
            ...
            img_sheared = mmcv.imshear(
                img, magnitude, direction=self.direction, border_value=self.pad_val, interpolation=self.interpolation
            )
    ...
    def __call__(self, results):
        ...
        for key in results.get("img_fields", ["img"]):
            ...
            img_translated = mmcv.imtranslate(
                img, offset, direction=self.direction, border_value=self.pad_val, interpolation=self.interpolation
            )
...
class Rotate(object):
    ...
    def __init__(
        self, angle, center=None, scale=1.0, pad_val=128, prob=0.5, random_negative_prob=0.5, interpolation="nearest"
    ):
        assert isinstance(angle, float), "The angle type must be float, but " f"got {type(angle)} instead."
        if isinstance(center, tuple):
            assert len(center) == 2, "center as a tuple must have 2 " f"elements, got {len(center)} elements instead."
        else:
            assert center is None, "The center type" f"must be tuple or None, got {type(center)} instead."
        assert isinstance(scale, float), "the scale type must be float, but " f"got {type(scale)} instead."
        if isinstance(pad_val, int):
            pad_val = tuple([pad_val] * 3)
        elif isinstance(pad_val, Sequence):
            assert len(pad_val) == 3, "pad_val as a tuple must have 3 " f"elements, got {len(pad_val)} instead."
...
class Solarize(object):
    ...
    def __init__(self, thr, prob=0.5):
        assert isinstance(thr, (int, float)), "The thr type must " f"be int or float, but got {type(thr)} instead."
        assert 0 <= prob <= 1.0, "The prob should be in range [0,1], " f"got {prob} instead."
...
class SolarizeAdd(object):
    ...
    def __init__(self, magnitude, thr=128, prob=0.5):
        assert isinstance(magnitude, (int, float)), (
            "The thr magnitude must " f"be int or float, but got {type(magnitude)} instead."
        )
        assert isinstance(thr, (int, float)), "The thr type must " f"be int or float, but got {type(thr)} instead."
        assert 0 <= prob <= 1.0, "The prob should be in range [0,1], " f"got {prob} instead."
    ...
    def __call__(self, results):
        ...
        for key in results.get("img_fields", ["img"]):
            img = results[key]
            img_solarized = np.where(img < self.thr, np.minimum(img + self.magnitude, 255), img)
            results[key] = img_solarized.astype(img.dtype)
        return results
...
class Cutout(object):
    ...
    def __init__(self, shape, pad_val=128, prob=0.5):
        if isinstance(shape, float):
            ...
            raise TypeError("shape must be of " f"type int, float or tuple, got {type(shape)} instead")
        if isinstance(pad_val, int):
            pad_val = tuple([pad_val] * 3)
        elif isinstance(pad_val, Sequence):
            assert len(pad_val) == 3, "pad_val as a tuple must have 3 " f"elements, got {len(pad_val)} instead."

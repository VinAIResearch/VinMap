...
class CIFAR10(BaseDataset):
    ...
    def load_annotations(self):
        ...
        if rank == 0 and not self._check_integrity():
            download_and_extract_archive(self.url, self.data_prefix, filename=self.filename, md5=self.tgz_md5)

        if world_size > 1:
            dist.barrier()
            assert self._check_integrity(), (
                "Shared storage seems unavailable. " f"Please download the dataset manually through {self.url}."
            )
    ...
    def _load_meta(self):
        path = os.path.join(self.data_prefix, self.base_folder, self.meta["filename"])

...
class StanfordCars(BaseDataset):
    ...
    def __init__(self, data_prefix: str, test_mode: bool, ann_file: Optional[str] = None, **kwargs):
        if test_mode:
            if ann_file is not None:
                self.test_ann_file = ann_file
            else:
                self.test_ann_file = osp.join(data_prefix, "devkit/cars_test_annos_withlabels.mat")
            data_prefix = osp.join(data_prefix, "cars_test")
        else:
            if ann_file is not None:
                self.train_ann_file = ann_file
            else:
                self.train_ann_file = osp.join(data_prefix, "devkit/cars_train_annos.mat")
            data_prefix = osp.join(data_prefix, "cars_train")
        super(StanfordCars, self).__init__(ann_file=ann_file, data_prefix=data_prefix, test_mode=test_mode, **kwargs)

    def load_annotations(self):
        try:
            import scipy.io as sio
        except ImportError:
            raise ImportError("please run `pip install scipy` to install package `scipy`.")

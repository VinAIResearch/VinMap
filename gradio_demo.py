...
def parse_args():
    parser = argparse.ArgumentParser(description="Release-24.03.12")
    ...
    parser.add_argument(
        "-map_weights", "--map_weights", default="../weight/effnetb4_imagemap.pth", help="Map Classification Weight"
    )
    parser.add_argument(
        "-cls_weights", "--cls_weights", default="../weight/effnetb4_vnmap.pth", help="VN Map Classification Weight"
    )
    parser.add_argument(
                ...
                for boundary_result in boundary_results:
                    ...
                    if x_tl < 0 or y_tl < 0 or x_br >= np_image.shape[1] or y_br >= np_image.shape[0]:
                        np_image = cv2.copyMakeBorder(
                            np_image, 500, 500, 500, 500, cv2.BORDER_CONSTANT, value=[0, 0, 0]
                        )
                        np_image = np_image[y_tl + 500 : y_br + 500, x_tl + 500 : x_br + 500]
                    else:
                        np_image = np_image[y_tl:y_br, x_tl:x_br]
...
if __name__ == "__main__":
    ...
    demo = gr.Interface(fn=greet, inputs=["image"], outputs=["text", "image"], share=True)

...
class SDMGRLoss(nn.Layer):
    ...
    def accuracy(self, pred, target, topk=1, thresh=None):
        ...
        correct = paddle.equal(pred_label, (target.reshape([1, -1]).expand_as(pred_label)))
        res = []
        for k in topk:
            correct_k = paddle.sum(correct[:k].reshape([-1]).astype("float32"), axis=0, keepdim=True)
    ...
    def forward(self, pred, batch):
        ...
        return dict(
            ...
            loss_node=loss_node,
            loss_edge=loss_edge,
            acc_node=self.accuracy(paddle.gather(node_preds, node_valids), paddle.gather(node_gts, node_valids)),
            acc_edge=self.accuracy(paddle.gather(edge_preds, edge_valids), paddle.gather(edge_gts, edge_valids)),
        )

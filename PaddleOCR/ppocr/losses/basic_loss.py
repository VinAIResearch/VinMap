...
class KLJSLoss(object):
    ...
    def __call__(self, p1, p2, reduction="mean", eps=1e-5):
        ...
        if self.mode.lower() == "kl":
            ...
            loss = paddle.multiply(p2, paddle.log((2 * p2 + eps) / (p1 + p2 + eps) + eps))
            loss += paddle.multiply(p1, paddle.log((2 * p1 + eps) / (p1 + p2 + eps) + eps))
            loss *= 0.5
        else:
...
class DKDLoss(nn.Layer):
    ...
    def forward(self, logits_student, logits_teacher, target, mask=None):
        ...
        pred_teacher_part2 = F.softmax(logits_teacher / self.temperature - 1000.0 * gt_mask, axis=1)
        log_pred_student_part2 = F.log_softmax(logits_student / self.temperature - 1000.0 * gt_mask, axis=1)

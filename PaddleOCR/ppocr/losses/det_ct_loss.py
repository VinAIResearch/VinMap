...
def ohem_single(score, gt_text, training_mask):
    ...
    selected_mask = paddle.cast(selected_mask.reshape((1, selected_mask.shape[0], selected_mask.shape[1])), "float32")
    return selected_mask
...
class SmoothL1Loss(nn.Layer):
    ...
    def select_single(self, distance, gt_instance, gt_kernel_instance, training_mask):
        ...
        with paddle.no_grad():
            ...
            off_points = paddle.cast(self.coord, "float32") + 10 * select_distance.transpose((1, 0))
            ...
            selected_mask = paddle.cast(selected_mask.reshape((1, -1, distance.shape[-1])), "int64")
    ...
    def forward(self, distances, gt_instances, gt_kernel_instances, training_masks, gt_distances, reduce=True):
        ...
        selected_training_masks = paddle.cast(paddle.concat(selected_training_masks, 0), "float32")

...
class DRRGLoss(nn.Layer):
    ...
    def balance_bce_loss(self, pred, gt, mask):
        ...
        balance_loss = (positive_loss + paddle.sum(negative_loss)) / (float(positive_count + negative_count) + 1e-5)
    ...
    def forward(self, preds, labels):
        ...
        gt_text_mask, gt_center_region_mask, gt_mask, gt_top_height_map, gt_bot_height_map, gt_sin_map, gt_cos_map = (
            labels[1:8]
        )
        ...
        if int(text_mask.sum()) > 0:
            loss_center_positive = paddle.sum(loss_center_map * text_mask) / paddle.sum(text_mask)
        else:
            loss_center_positive = paddle.to_tensor(0.0)
        loss_center_negative = paddle.sum(loss_center_map * negative_text_mask) / paddle.sum(negative_text_mask)
        ...
        if int(center_mask.sum()) > 0:
            ...
            loss_height = paddle.sum((paddle.log(gt_height + 1) * (loss_top + loss_bot)) * center_mask) / paddle.sum(
                center_mask
            )

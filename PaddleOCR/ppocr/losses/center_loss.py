...
class CenterLoss(nn.Layer):
    ...
    def __init__(self, num_classes=6625, feat_dim=96, center_file_path=None):
        ...
        self.centers = paddle.randn(shape=[self.num_classes, self.feat_dim]).astype("float64")
    ...
    def __call__(self, predicts, batch):
        ...
        feats_reshape = paddle.reshape(features, [-1, features.shape[-1]]).astype("float64")
        ...
        square_center = paddle.expand(square_center, [self.num_classes, batch_size]).astype("float64")
        square_center = paddle.transpose(square_center, [1, 0])

        distmat = paddle.add(square_feat, square_center)
        feat_dot_center = paddle.matmul(feats_reshape, paddle.transpose(self.centers, [1, 0]))
        distmat = distmat - 2.0 * feat_dot_center

        # generate the mask
        classes = paddle.arange(self.num_classes).astype("int64")
        label = paddle.expand(paddle.unsqueeze(label, 1), (batch_size, self.num_classes))
        mask = paddle.equal(paddle.expand(classes, [batch_size, self.num_classes]), label).astype("float64")

...
class FCELoss(nn.Layer):
    ...
    def forward_single(self, pred, gt):
        ...
        tr_train_mask2 = paddle.concat([tr_train_mask.unsqueeze(1), tr_train_mask.unsqueeze(1)], axis=1)
        # tr loss
        loss_tr = self.ohem(tr_pred, tr_mask, train_mask)
        # tcl loss
        loss_tcl = paddle.to_tensor(0.0).astype("float32")
        tr_neg_mask = tr_train_mask.logical_not()
        tr_neg_mask2 = paddle.concat([tr_neg_mask.unsqueeze(1), tr_neg_mask.unsqueeze(1)], axis=1)
        ...
        if tr_train_mask.sum().item() > 0:
            ...
            tr_train_mask3 = paddle.concat([tr_train_mask.unsqueeze(1) for i in range(dim)], axis=1)
    ...
    def fourier2poly(self, real_maps, imag_maps):
        ...
        k_vect = paddle.arange(-self.fourier_degree, self.fourier_degree + 1, dtype="float32").reshape([-1, 1])

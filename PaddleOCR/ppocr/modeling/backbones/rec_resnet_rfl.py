...
class ResNetBase(nn.Layer):
    def __init__(self, in_channels, out_channels, block, layers):
        ...
        self.conv0_1 = nn.Conv2D(
            in_channels, int(out_channels / 16), kernel_size=3, stride=1, padding=1, bias_attr=False
        )
        self.bn0_1 = nn.BatchNorm(int(out_channels / 16))
        self.conv0_2 = nn.Conv2D(
            int(out_channels / 16), self.inplanes, kernel_size=3, stride=1, padding=1, bias_attr=False
        )
        ...
        self.conv1 = nn.Conv2D(
            self.out_channels_block[0], self.out_channels_block[0], kernel_size=3, stride=1, padding=1, bias_attr=False
        )
        ...
        self.conv2 = nn.Conv2D(
            self.out_channels_block[1], self.out_channels_block[1], kernel_size=3, stride=1, padding=1, bias_attr=False
        )

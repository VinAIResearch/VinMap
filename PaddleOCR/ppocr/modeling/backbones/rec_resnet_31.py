...
def conv3x3(in_channel, out_channel, stride=1, conv_weight_attr=None):
    return nn.Conv2D(
        in_channel, out_channel, kernel_size=3, stride=stride, padding=1, weight_attr=conv_weight_attr, bias_attr=False
    )
    ...
    def __init__(self, in_channels, channels, stride=1, downsample=False, conv_weight_attr=None, bn_weight_attr=None):
        ...
        if downsample:
            self.downsample = nn.Sequential(
                nn.Conv2D(
                    in_channels, channels * self.expansion, 1, stride, weight_attr=conv_weight_attr, bias_attr=False
                ),

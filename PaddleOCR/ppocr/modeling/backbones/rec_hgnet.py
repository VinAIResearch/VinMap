...
class ESEModule(nn.Layer):
    def __init__(self, channels):
        ...
        self.conv = Conv2D(in_channels=channels, out_channels=channels, kernel_size=1, stride=1, padding=0)
...
class HG_Block(nn.Layer):
    def __init__(
        ...
        self.aggregation_conv = ConvBNAct(
            in_channels=total_channels, out_channels=out_channels, kernel_size=1, stride=1
        )
...
class PPHGNet(nn.Layer):
    ...
    def __init__(
        ...
        for block_id, k in enumerate(stage_config):
            ...
            self.stages.append(
                HG_Stage(in_channels, mid_channels, out_channels, block_num, layer_num, downsample, stride)
            )
...
def PPHGNet_tiny(pretrained=False, use_ssld=False, **kwargs):
    ...
    model = PPHGNet(stem_channels=[48, 48, 96], stage_config=stage_config, layer_num=5, **kwargs)
    return model
...
def PPHGNet_base(pretrained=False, use_ssld=True, **kwargs):
    ...
    model = PPHGNet(stem_channels=[96, 96, 160], stage_config=stage_config, layer_num=7, dropout_prob=0.2, **kwargs)
    return model

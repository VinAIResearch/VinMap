...
class SpatialSepConvSF(nn.Layer):
    def __init__(self, inp, oups, kernel_size, stride):
        ...
        self.conv = nn.Sequential(
            nn.Conv2D(inp, oup1, (kernel_size, 1), (stride, 1), (kernel_size // 2, 0), bias_attr=False, groups=1),
            nn.BatchNorm2D(oup1),
            nn.Conv2D(
                oup1, oup1 * oup2, (1, kernel_size), (1, stride), (0, kernel_size // 2), bias_attr=False, groups=oup1
            ),
...
class DepthSpatialSepConv(nn.Layer):
    def __init__(self, inp, expand, kernel_size, stride):
        ...
        self.conv = nn.Sequential(
            nn.Conv2D(
                inp, inp * exp1, (kernel_size, 1), (stride, 1), (kernel_size // 2, 0), bias_attr=False, groups=inp
            ),
            nn.BatchNorm2D(inp * exp1),
            nn.Conv2D(hidden_dim, oup, (1, kernel_size), 1, (0, kernel_size // 2), bias_attr=False, groups=hidden_dim),
            nn.BatchNorm2D(oup),
        )
...
class GroupConv(nn.Layer):
    def __init__(self, inp, oup, groups=2):
        ...
        self.groups = groups
        self.conv = nn.Sequential(
            nn.Conv2D(inp, oup, 1, 1, 0, bias_attr=False, groups=self.groups[0]), nn.BatchNorm2D(oup)
        )
        ...
        if gs1[0] == 0:
            ...
            self.layers = nn.Sequential(
                ...
                (
                    ...
                    else nn.Sequential() if y1 == 0 and y2 == 0 else ChannelShuffle(hidden_dim2 // 2)
                ),
                GroupConv(hidden_dim2, oup, (g1, g2)),
                (
                    DYShiftMax(

...
class STN(nn.Layer):
    def __init__(self, in_channels, num_ctrlpoints, activation="none"):
        ...
        self.stn_fc1 = nn.Sequential(
            nn.Linear(2 * 256, 512, weight_attr=nn.initializer.Normal(0, 0.001), bias_attr=nn.initializer.Constant(0)),
            nn.BatchNorm1D(512),
            nn.ReLU(),
        )
    ...
    def init_stn(self):
        ...
        fc2_bias = paddle.reshape(ctrl_points, shape=[ctrl_points.shape[0] * ctrl_points.shape[1]])
        return fc2_bias
...
class STN_ON(nn.Layer):
    ...
    def forward(self, image):
        stn_input = paddle.nn.functional.interpolate(image, self.tps_inputsize, mode="bilinear", align_corners=True)

    ...
    def __init__(self, in_channels, num_fiducial, loc_lr, model_name):
        ...
        self.fc1 = nn.Linear(
            ...
            weight_attr=ParamAttr(
                learning_rate=loc_lr, name=name + "_w", initializer=nn.initializer.Uniform(-stdv, stdv)
            ),
        ...
        param_attr = ParamAttr(
            learning_rate=loc_lr, initializer=nn.initializer.Assign(np.zeros([fc_dim, F * 2])), name=name + "_w"
        )
        bias_attr = ParamAttr(learning_rate=loc_lr, initializer=nn.initializer.Assign(initial_bias), name=name + "_b")
        self.fc2 = nn.Linear(fc_dim, F * 2, weight_attr=param_attr, bias_attr=bias_attr, name=name)
...
class GridGenerator(nn.Layer):
    def __init__(self, in_channels, num_fiducial):
        ...
        bias_attr = ParamAttr(learning_rate=0.0, initializer=initializer, name=name + "_b")
        self.fc = nn.Linear(in_channels, 6, weight_attr=param_attr, bias_attr=bias_attr, name=name)
    ...
    def forward(self, batch_C_prime, I_r_size):
        ...
        batch_C_prime_with_zeros = paddle.concat([batch_C_prime, batch_C_ex_part_tensor], axis=1)
    ...
    def build_P_paddle(self, I_r_size):
        ...
        I_r_grid_x = (paddle.arange(-I_r_width, I_r_width, 2, dtype="float64") + 1.0) / paddle.to_tensor(
            np.array([I_r_width])
        )

        I_r_grid_y = (paddle.arange(-I_r_height, I_r_height, 2, dtype="float64") + 1.0) / paddle.to_tensor(
            np.array([I_r_height])
        )
    ...
    def build_inv_delta_C_paddle(self, C):
        ...
        delta_C = paddle.concat(  # F+3 x F+3
            [
                paddle.concat([paddle.ones((F, 1), dtype="float64"), C, hat_C], axis=1),  # F x F+3
                paddle.concat(
                    [paddle.zeros((2, 3), dtype="float64"), paddle.transpose(C, perm=[1, 0])], axis=1
                ),  # 2 x F+3
                paddle.concat(
                    [paddle.zeros((1, 3), dtype="float64"), paddle.ones((1, F), dtype="float64")], axis=1
                ),  # 1 x F+3
            ],
            axis=0,
        )

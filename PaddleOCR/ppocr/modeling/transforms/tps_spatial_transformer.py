...
def compute_partial_repr(input_points, control_points):
    ...
    pairwise_diff = paddle.reshape(input_points, shape=[N, 1, 2]) - paddle.reshape(control_points, shape=[1, M, 2])
...
class TPSSpatialTransformer(nn.Layer):
    def __init__(self, output_image_size=None, num_control_points=None, margins=None):
        ...
        target_control_partial_repr = paddle.cast(target_control_partial_repr, forward_kernel.dtype)
        ...
        target_coordinate = paddle.concat([X, Y], axis=1)  # convert from (y, x) to (x, y)
        target_coordinate_partial_repr = compute_partial_repr(target_coordinate, target_control_points)
        target_coordinate_repr = paddle.concat(
            [target_coordinate_partial_repr, paddle.ones(shape=[HW, 1]), target_coordinate], axis=1
        )
    ...
    def forward(self, input, source_control_points):
        ...
        Y = paddle.concat([source_control_points.astype(padding_matrix.dtype), padding_matrix], 1)
        ...
        grid = paddle.reshape(source_coordinate, shape=[-1, self.target_height, self.target_width, 2])
        grid = paddle.clip(grid, 0, 1)  # the source_control_points may be out of [0, 1].

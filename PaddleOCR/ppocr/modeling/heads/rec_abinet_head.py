...
class PositionAttention(nn.Layer):
    ...
    def forward(self, x):
        ...
        # TODO q=f(q,k)
        zeros = paddle.zeros((B, self.max_length, C), dtype=x.dtype)  # (T, N, C)
    ...
    def forward(self, x, targets=None):
        ...
        if self.use_lang:
            ...
            for i in range(self.iter_size):
                ...
                lengths = paddle.clip(lengths, 2, self.max_length)  # TODO:move to langauge model
...
def _get_mask(length, max_length):
    ...
    diag_mask = paddle.diag(paddle.full([max_length], "-inf", dtype=paddle.float32), offset=0, name=None)

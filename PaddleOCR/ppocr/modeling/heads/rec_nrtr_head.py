...
from ppocr.modeling.backbones.rec_svtrnet import Mlp, ones_, zeros_
...
class Transformer(nn.Layer):
    ...
    def __init__(
        ...
        self.embedding = Embeddings(
            d_model=d_model, vocab=self.out_channels, padding_idx=0, scale_embedding=scale_embedding
        )
    ...
    def forward_test(self, src):
        ...
        for len_dec_seq in range(1, paddle.to_tensor(self.max_len)):
            ...
            tgt_mask = self.generate_square_subsequent_mask(paddle.shape(dec_seq_embed)[1])
            tgt = dec_seq_embed
            for decoder_layer in self.decoder:
                tgt = decoder_layer(tgt, memory, self_mask=tgt_mask)
            dec_output = tgt
            ...
            if paddle.equal_all(preds_idx, paddle.full(paddle.shape(preds_idx), 3, dtype="int64")):
                break
            preds_prob = paddle.max(word_prob, axis=-1)
            dec_seq = paddle.concat([dec_seq, paddle.reshape(preds_idx, [-1, 1])], axis=1)
            dec_prob = paddle.concat([dec_prob, paddle.reshape(preds_prob, [-1, 1])], axis=1)
    ...
    def forward_beam(self, images):
        ...
        def beam_decode_step(inst_dec_beams, len_dec_seq, enc_output, inst_idx_to_position_map, n_bm):
            ...
            def predict_word(dec_seq, enc_output, n_active_inst, n_bm):
                ...
                tgt_mask = self.generate_square_subsequent_mask(paddle.shape(dec_seq)[1])
                tgt = dec_seq
                for decoder_layer in self.decoder:
        ...
        with paddle.no_grad():
            ...
            for len_dec_seq in range(1, paddle.to_tensor(self.max_len)):
                ...
                active_inst_idx_list = beam_decode_step(
                    inst_dec_beams, len_dec_seq, src_enc_copy, inst_idx_to_position_map, n_bm
                )
        ...
        return [paddle.to_tensor(np.array(result_hyp), dtype=paddle.int64), paddle.to_tensor(hyp_scores)]
...
class TransformerBlock(nn.Layer):
    def __init__(
        ...
        self.mlp = Mlp(
            in_features=d_model, hidden_features=dim_feedforward, act_layer=nn.ReLU, drop=residual_dropout_rate
        )
...
class PositionalEncoding_2d(nn.Layer):
    ...
    def forward(self, x):
        ...
        x = x + w_pe + h_pe
        x = paddle.transpose(paddle.reshape(x, [x.shape[0], x.shape[1], x.shape[2] * x.shape[3]]), [2, 0, 1])
...
class Beam:
    ...
    def sort_scores(self):
        "Sort the scores."
        return self.scores, paddle.to_tensor([i for i in range(int(self.scores.shape[0]))], dtype="int32")

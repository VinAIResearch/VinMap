    ...
    def forward(self, feat, img_metas=None):
        ...
        if valid_ratios is not None:
            ...
            for i in range(paddle.shape(valid_ratios)[0]):
                valid_step = paddle.minimum(T, paddle.ceil(valid_ratios[i] * T).astype("int32")) - 1
...
class ParallelSARDecoder(BaseDecoder):
    ...
    def _2d_attention(self, decoder_input, feat, holistic_feat, valid_ratios=None):
        ...
        if valid_ratios is not None:
            ...
            for i in range(paddle.shape(valid_ratios)[0]):
                valid_width = paddle.minimum(w, paddle.ceil(valid_ratios[i] * w).astype("int32"))
                if valid_width < w:
        ...
        if self.pred_concat:
            ...
            y = self.prediction(paddle.concat((y, attn_feat.astype(y.dtype), holistic_feat.astype(y.dtype)), 2))
        else:

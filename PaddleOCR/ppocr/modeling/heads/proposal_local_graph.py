...
class ProposalLocalGraphs:
    ...
    def generate_local_graphs(self, sorted_dist_inds, node_feats):
        ...
        for graph_ind, pivot_knn in enumerate(pivot_knns):
            ...
            knn_inds = paddle.cast(paddle.to_tensor([node2ind_map[i] for i in pivot_knn[1:]]), "int64")
            pivot_feats = node_feats[pivot_ind]
            normalized_feats = node_feats[paddle.to_tensor(pivot_local_graph)] - pivot_feats
            ...
            pad_adjacent_matrix[:num_nodes, :num_nodes] = paddle.cast(paddle.to_tensor(adjacent_matrix), "float32")
            ...
            local_graph_nodes = paddle.concat(
                [local_graph_nodes, paddle.zeros([num_max_nodes - num_nodes], dtype="int64")], axis=-1
            )

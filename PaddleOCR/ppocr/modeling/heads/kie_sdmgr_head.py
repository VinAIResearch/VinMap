...
class SDMGRHead(nn.Layer):
    ...
    def forward(self, input, targets):
        ...
        all_nodes = paddle.concat(
            [paddle.concat([text, paddle.zeros((text.shape[0], max_num - text.shape[1]))], -1) for text in texts]
        )
        ...
        temp_all_nums = paddle.expand(
            temp_all_nums, [temp_all_nums.shape[0], temp_all_nums.shape[1], rnn_nodes.shape[-1]]
        )
        ...
        one_hot = paddle.multiply(temp_all_nodes, one_hot.astype("float32")).sum(axis=1, keepdim=True)
        ...
        all_edges = paddle.concat([rel.reshape([-1, rel.shape[-1]]) for rel in relations])
...
class GNNLayer(nn.Layer):
    ...
    def forward(self, nodes, edges, nums):
        ...
        for num in nums:
            residual = F.softmax(
                -paddle.eye(num).unsqueeze(-1) * 1e9 + coefs[start : start + num**2].reshape([num, num, -1]), 1
            )

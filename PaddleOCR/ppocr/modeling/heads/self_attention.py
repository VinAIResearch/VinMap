...
class MultiHeadAttention(nn.Layer):
    ...
    def __init__(self, d_key, d_value, d_model, n_head=1, dropout_rate=0.0):
        ...
        self.q_fc = paddle.nn.Linear(in_features=d_model, out_features=d_key * n_head, bias_attr=False)
        self.k_fc = paddle.nn.Linear(in_features=d_model, out_features=d_key * n_head, bias_attr=False)
        self.v_fc = paddle.nn.Linear(in_features=d_model, out_features=d_value * n_head, bias_attr=False)
        self.proj_fc = paddle.nn.Linear(in_features=d_value * n_head, out_features=d_model, bias_attr=False)
    ...
    def __init__(self, process_cmd, d_model, dropout_rate):
        ...
        for cmd in self.process_cmd:
            if cmd == "a":  # add residual connection
                ...
                self.functors.append(
                    lambda x: F.dropout(x, p=dropout_rate, mode="downscale_in_infer") if dropout_rate else x
                )
...
class PrepareEncoder(nn.Layer):
    def __init__(
        ...
        self.emb = paddle.nn.Embedding(num_embeddings=self.src_max_len, embedding_dim=self.src_emb_dim)
        self.dropout_rate = dropout_rate
    ...
    def __init__(
        ...
        self.emb0 = paddle.nn.Embedding(
            ...
            weight_attr=paddle.ParamAttr(
                name=word_emb_param_name, initializer=nn.initializer.Normal(0.0, src_emb_dim**-0.5)
            ),
        )

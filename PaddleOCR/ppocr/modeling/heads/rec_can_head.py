...
class CountingDecoder(nn.Layer):
    def __init__(self, in_channel, out_channel, kernel_size):
        ...
        self.pred_layer = nn.Sequential(nn.Conv2D(512, self.out_channel, kernel_size=1, bias_attr=False), nn.Sigmoid())
...
class PositionEmbeddingSine(nn.Layer):
    ...
    def forward(self, x, mask):
        ...
        pos_x = paddle.flatten(
            paddle.stack([paddle.sin(pos_x[:, :, :, 0::2]), paddle.cos(pos_x[:, :, :, 1::2])], axis=4), 3
        )
        pos_y = paddle.flatten(
            paddle.stack([paddle.sin(pos_y[:, :, :, 0::2]), paddle.cos(pos_y[:, :, :, 1::2])], axis=4), 3
        )
    ...
    def forward(self, cnn_features, labels, counting_preds, images_mask):
        ...
        for i in range(num_steps):
            ...
            if self.is_train:
                ...
                word = paddle.multiply(word, labels[:, i])  # labels are oneslike tensor in infer/predict mode

        return word_probs

    def init_hidden(self, features, feature_mask):
        average = paddle.sum(paddle.sum(features * feature_mask, axis=-1), axis=-1) / paddle.sum(
...
class Attention(nn.Layer):
    ...
    def forward(self, cnn_features, cnn_features_trans, hidden, alpha_sum, image_mask=None):
        ...
        alpha_score = paddle.tanh(
            paddle.unsqueeze(query, [1, 2]) + coverage_alpha + paddle.transpose(cnn_features_trans, [0, 2, 3, 1])
        )

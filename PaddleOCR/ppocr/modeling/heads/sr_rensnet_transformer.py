...
def subsequent_mask(size):
    ...
    mask_inf = paddle.triu(paddle.full(shape=[1, size, size], dtype="float32", fill_value="-inf"), diagonal=1)
    mask = mask + mask_inf
...
class LayerNorm(nn.Layer):
    ...
    def __init__(self, features, eps=1e-6):
        ...
        self.a_2 = self.create_parameter(shape=[features], default_initializer=paddle.nn.initializer.Constant(1.0))
        self.b_2 = self.create_parameter(shape=[features], default_initializer=paddle.nn.initializer.Constant(0.0))
        self.eps = eps

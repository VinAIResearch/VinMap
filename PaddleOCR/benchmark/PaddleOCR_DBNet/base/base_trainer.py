...
class BaseTrainer:
    def __init__(self, config, model, criterion, train_loader, validate_loader, metric_cls, post_process=None):
        config["trainer"]["output_dir"] = os.path.join(
            str(pathlib.Path(os.path.abspath(__name__)).parent), config["trainer"]["output_dir"]
        )
        ...
        if self.amp:
            ...
            self.model, self.optimizer = paddle.amp.decorate(
                models=self.model, optimizers=self.optimizer, level=self.amp.get("amp_level", "O2")
            )
    ...
    def apply_to_static(self, model):
        ...
        if support_to_static:
            ...
            self.logger_info("Successfully to apply @to_static with specs: {}".format(specs))
        return model
    ...
    def _initialize_optimizer(self):
        self.optimizer = self._initialize(
            "optimizer", paddle.optimizer, parameters=self.model.parameters(), learning_rate=self.lr_scheduler
        )

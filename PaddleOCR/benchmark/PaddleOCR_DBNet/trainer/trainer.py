...
from utils import Polynomial, cal_text_score, profiler, runningScore
...
class Trainer(BaseTrainer):
    def __init__(
        ...
        super(Trainer, self).__init__(
            config, model, criterion, train_loader, validate_loader, metric_cls, post_process
        )
    ...
    def _on_epoch_finish(self):
        ...
        if paddle.distributed.get_rank() == 0:
            ...
            save_best = False
            if (
                self.validate_loader is not None and self.metric_cls is not None and self.enable_eval
            ):  # 使用f1作为最优模型指标
                ...
                self.logger_info(
                    "test: recall: {:.6f}, precision: {:.6f}, hmean: {:.6f}".format(recall, precision, hmean)
                )

        ...
        for request in requests:
            ...
            for i_batch in range(len(results)):
                ...
                inference_request = pb_utils.InferenceRequest(
                    model_name="cls_pp", requested_output_names=["cls_labels", "cls_scores"], inputs=[cls_input_tensor]
                )
                ...
                if inference_response.has_error():
                    ...
                    cls_labels = pb_utils.get_output_tensor_by_name(inference_response, "cls_labels")
                    cls_labels = cls_labels.as_numpy()

                    cls_scores = pb_utils.get_output_tensor_by_name(inference_response, "cls_scores")
                    cls_scores = cls_scores.as_numpy()

                for index in range(len(image_list)):
                    if cls_labels[index] == 1 and cls_scores[index] > self.cls_threshold:
                        image_list[index] = cv2.rotate(image_list[index].astype(np.float32), 1)

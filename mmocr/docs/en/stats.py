...
for f in files:
    ...
    paperlist = "\n".join(sorted(f"    - [{t}] {paperlinks[x]}" for t, x in papers))
    # count configs
    configs = set(x.lower().strip() for x in re.findall(r"https.*configs/.*\.py", content))

    # count ckpts
    ckpts = set(x.lower().strip() for x in re.findall(r"https://download.*\.pth", content) if "mmocr" in x)

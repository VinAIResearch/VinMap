...
def test_loadtextannotation():
    ...
    loader = LoadTextAnnotations(with_bbox, with_label, with_mask, with_seg, poly2mask, use_img_shape=True)
    tmp_results = copy.deepcopy(results)
    output = loader._load_masks(tmp_results)
    assert len(output["gt_masks_ignore"]) == 4
    assert np.allclose(output["gt_masks_ignore"].masks[0], [[499, 94, 531, 94, 531, 124, 499, 124]])
    ...
    loader = LoadTextAnnotations(with_bbox, with_label, with_mask, with_seg, poly2mask=True, use_img_shape=True)
    tmp_results = copy.deepcopy(results)

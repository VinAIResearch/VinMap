...
def test_ocr_segm_targets():
    ...
    label_convertor = dict(type="SegConvertor", dict_file=dict_file, with_unknown=True, lower=True)
    # test init
    ...
    with pytest.raises(AssertionError):
        ocr_seg_tgt.generate_kernels(8, pad_size, char_boxes, char_idxs, 0.5, True)
    with pytest.raises(AssertionError):
        ocr_seg_tgt.generate_kernels(img_size, pad_size, [2, 2, 6, 6], char_idxs, 0.5, True)
    with pytest.raises(AssertionError):
        ocr_seg_tgt.generate_kernels(img_size, pad_size, char_boxes, 2, 0.5, True)

    attn_tgt = ocr_seg_tgt.generate_kernels(img_size, pad_size, char_boxes, char_idxs, 0.5, binary=True)
    expect_attn_tgt = [
    ...
    segm_tgt = ocr_seg_tgt.generate_kernels(img_size, pad_size, char_boxes, char_idxs, 0.5, binary=False)
    expect_segm_tgt = [
        [0, 0, 0, 0, 0, 0, 0, 0, 255, 255],
        [0, 0, 0, 0, 0, 0, 0, 0, 255, 255],
        [0, 0, 0, 0, 0, 0, 0, 0, 255, 255],
    ...
    assert np.allclose(results["gt_kernels"].masks[0], np.array(expect_attn_tgt, dtype=np.int32))
    assert np.allclose(results["gt_kernels"].masks[1], np.array(expect_segm_tgt, dtype=np.int32))

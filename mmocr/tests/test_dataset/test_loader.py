...
import pytest
from mmocr.datasets.utils.backend import HardDiskAnnFileBackend, HTTPAnnFileBackend, PetrelAnnFileBackend
...
def test_loader():
    ...
    text_loader = AnnFileLoader(ann_file, json_parser, repeat=1, file_format="txt")
    assert text_loader[0] == {"filename": "sample1.jpg", "text": "hello"}
    ...
    recog2lmdb(img_root=None, label_path=ann_file, label_only=True, output=lmdb_file, lmdb_map_size=102400)

    parser = dict(type="LineJsonParser", keys=["filename", "text"])
    lmdb_loader = AnnFileLoader(lmdb_file, parser, repeat=1, file_format="lmdb")

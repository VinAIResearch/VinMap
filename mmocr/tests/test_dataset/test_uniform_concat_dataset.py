...
def test_uniform_concat_dataset_pipeline():
    ...
    tmp_dataset = UniformConcatDataset(datasets=[copy_train1, copy_train2], pipeline=pipeline1, force_apply=True)

    assert len(tmp_dataset) == 2 * len(list_from_file(ann_file))
    assert len(tmp_dataset.datasets[0].pipeline.transforms) == len(tmp_dataset.datasets[1].pipeline.transforms)
    ...
    copy_train2 = copy.deepcopy(train2)
    tmp_dataset = UniformConcatDataset(datasets=[[copy_train2], [copy_train2]], pipeline=None)
    assert len(tmp_dataset.datasets[0].pipeline.transforms) == len(pipeline2)

    # pipeline is 2d list
    copy_train1 = copy.deepcopy(train1)
    copy_train2 = copy.deepcopy(train2)
    tmp_dataset = UniformConcatDataset(datasets=[[copy_train1], [copy_train2]], pipeline=[pipeline1, pipeline2])
...
def test_uniform_concat_dataset_eval():
    ...
    with pytest.raises(NotImplementedError):
        ...
        UniformConcatDataset([dict(type="DummyDataset"), dict(type="DummyDataset2")], show_mean_scores=True)

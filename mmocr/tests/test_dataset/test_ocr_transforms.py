...
def test_resize_ocr():
    ...
    rci = transforms.ResizeOCR(32, min_width=32, max_width=160, keep_aspect_ratio=True)
    ...
    rci = transforms.ResizeOCR(32, min_width=32, max_width=160, keep_aspect_ratio=False)
    results = {"img_shape": input_img.shape, "img": input_img}
    results = rci(results)
    assert math.isclose(results["valid_ratio"], 1)

    # test img_pad_value
    rci = transforms.ResizeOCR(32, min_width=32, max_width=160, keep_aspect_ratio=True, img_pad_value=(127, 127, 127))
    results = {"img_shape": input_img.shape, "img": input_img}
    results = rci(results)
...
def test_online_crop(mock_random):
    ...
    results = {
        "img": src_img,
        "img_info": {"x1": "20", "y1": "20", "x2": "40", "y2": "20", "x3": "40", "y3": "40", "x4": "20", "y4": "40"},
    }

...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="skip if on cpu device")
def test_detector_wrapper():
    ...
    cfg = dict(
        model=dict(
            ...
            neck=dict(type="FPNC", in_channels=[64, 128, 256, 512], lateral_channels=256),
            bbox_head=dict(
                type="DBHead",
                text_repr_type="quad",
                in_channels=256,
...
def test_recognizer_wrapper():
    ...
    cfg = dict(
        label_convertor=dict(type="CTCConvertor", dict_type="DICT36", with_unknown=False, lower=True),
        model=dict(
            type="CRNNNet",
            preprocessor=None,
    ...
    pytorch_model.forward = partial(pytorch_model.forward, img_metas=img_metas, return_loss=False, rescale=True)
    with tempfile.TemporaryDirectory() as tmpdirname:
        onnx_path = f"{tmpdirname}/tmp.onnx"
        with torch.no_grad():
            torch.onnx.export(

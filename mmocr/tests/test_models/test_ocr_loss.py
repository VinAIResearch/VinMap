...
from mmocr.models.textrecog.losses import ABILoss, CELoss, CTCLoss, SARLoss, TFLoss
...
def test_ctc_loss():
    ...
    targets_dict = {"flatten_targets": torch.IntTensor([1, 2, 3, 4, 5]), "target_lengths": torch.LongTensor([2, 3])}

    losses = ctc_loss(outputs, targets_dict)
    assert isinstance(losses, dict)
    assert "loss_ctc" in losses
    assert torch.allclose(losses["loss_ctc"], torch.tensor(losses["loss_ctc"].item()).float())
...
def test_ce_loss():
    ...
    targets_dict = {"padded_targets": torch.LongTensor([[1, 2, 3, 4, 0, 0, 0, 0, 0, 0]])}
...
def test_abi_loss():
    ...
    outputs = dict(
        ...
        out_decs=[dict(logits=torch.randn(2, 10, 90)), dict(logits=torch.randn(2, 10, 90))],
        out_fusers=[dict(logits=torch.randn(2, 10, 90)), dict(logits=torch.randn(2, 10, 90))],
    )
    targets_dict = {
        "padded_targets": torch.LongTensor([[1, 2, 3, 4, 0, 0, 0, 0, 0, 0]]),
        "targets": [torch.LongTensor([1, 2, 3, 4]), torch.LongTensor([1, 2, 3])],
    }

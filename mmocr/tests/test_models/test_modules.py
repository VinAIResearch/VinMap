...
def test_local_graph_forward_train():
    ...
    local_graph_generator = LocalGraphs((4, 4), 3, geo_feat_len, 1.0, pooling_out_size, 0.5)
    ...
    (node_feats, adjacent_matrix, knn_inds, linkage_labels) = local_graph_generator(feature_maps, comp_attribs)
    ...
    assert node_feats.size()[0] == adjacent_matrix.size()[0] == knn_inds.size()[0] == linkage_labels.size()[0]
    assert node_feats.size()[1] == adjacent_matrix.size()[1] == adjacent_matrix.size()[2]
...
def test_local_graph_forward_test():
    ...
    local_graph_generator = ProposalLocalGraphs(
        (4, 4), 2, geo_feat_len, 1.0, pooling_out_size, 0.1, 3.0, 6.0, 1.0, 0.5, 0.3, 0.5, 0.5, 2
    )
    ...
    (node_feats, adjacent_matrices, knn_inds, local_graphs, text_comps) = graph_data
    ...
    assert (
        node_feats.size()[0]
        == adjacent_matrices.size()[0]
        == knn_inds.size()[0]
        == local_graphs.size()[0]
        == text_comps.shape[0]
    )
    assert node_feats.size()[1] == adjacent_matrices.size()[1] == adjacent_matrices.size()[2] == local_graphs.size()[1]
    assert node_feats.size()[-1] == feat_len
    ...
    local_graph_generator = ProposalLocalGraphs(
        (4, 4), 2, geo_feat_len, 1.0, pooling_out_size, 0.1, 8.0, 20.0, 1.0, 0.5, 0.3, 0.5, 0.5, 2
    )
    maps[:, 1, 78:82, 78:82] = 10.0
    none_flag, _ = local_graph_generator(maps, feature_maps)
    assert none_flag

    # test proposal local graphs with text components out of text region
    maps[:, 0, 60:100, 50:170] = -10.0
    maps[:, 0, 78:82, 78:82] = 10.0
    none_flag, _ = local_graph_generator(maps, feature_maps)
    assert none_flag
...
def test_gcn():
    ...
    node_feat = torch.randn((num_local_graphs, num_max_graph_nodes, input_feat_len))
    adjacent_matrix = torch.rand((num_local_graphs, num_max_graph_nodes, num_max_graph_nodes))

...
def test_base_label_convertor():
    ...
    with open(dict_file, "w") as fw:
        fw.write("a\nb\nc \n")
    with pytest.raises(ValueError, match="Expect each line has 0 or 1 character, got 2" " characters at line 3"):
        label_convertor = BaseConvertor(dict_file=dict_file)
    with open(dict_file, "w") as fw:
        fw.write("   \n")
    ...
    with pytest.raises(NotImplementedError, match="Dict type DICT100 is not supported"):

...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "https://download.openmmlab.com/mmocr/textrecog/" "crnn/crnn_academic-a723a1c5.pth",
        ),
        (
            "DB_r18",
            "CRNN",
            ...
            [
                ...
                "dbnet/dbnet_r18_fpnc_sbn_1200e_icdar2015_20210329-ba3ab597.pth",
                "https://download.openmmlab.com/mmocr/textrecog/" "crnn/crnn_academic-a723a1c5.pth",
                "https://download.openmmlab.com/mmocr/kie/" "sdmgr/sdmgr_unet16_60e_wildreceipt_20220706-57c220a6.pth",
            ],
        ),
        (
            "DB_r18",
...
def test_ocr_init(
    mock_loading, mock_config, mock_build_detector, mock_init_detector, det, recog, kie, config_dir, gt_cfg, gt_ckpt
):

    def loadcheckpoint_assert(*args, **kwargs):
        assert args[1] == gt_ckpt[-1]
        assert kwargs["map_location"] == torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    with mock.patch("mmocr.utils.ocr.revert_sync_batchnorm"):
        ...
        calls = [mock.call(gt_cfg[i], gt_ckpt[i], device=device) for i in i_range]
...
@mock.patch("mmocr.utils.ocr.Config.fromfile")
@mock.patch("mmocr.utils.ocr.load_checkpoint")
def test_ocr_init_customize_config(
    mock_loading,
    mock_config,
    mock_build_detector,
    mock_init_detector,
...
def test_readtext(mock_kiedataset):
    ...
    assert all(
        [
            np.allclose(full_batch_det_res[i]["boundary_result"], det_res[i]["boundary_result"])
            for i in range(len(full_batch_det_res))
        ]
    )
    assert all(
        [
            np.allclose(batch_det_res[i]["boundary_result"], det_res[i]["boundary_result"])
    ...
    assert np.all(
        [np.allclose(batch_recog_res[i]["score"], recog_res[i]["score"]) for i in range(len(full_batch_recog_res))]
    )
...
@pytest.mark.parametrize(
    "det, recog, target",
    [
        ("Tesseract", None, {"boundary_result": [[0, 0, 1, 0, 1, 1, 0, 1, 1.0]]}),
        (
            "Tesseract",
            "Tesseract",
            {"result": [{"box": [0, 0, 1, 0, 1, 1, 0, 1], "box_score": 1.0, "text": "text", "text_score": 0.5}]},
        ),
        (None, "Tesseract", {"text": "text", "score": 0.5}),
    ],
)
@mock.patch("mmocr.utils.ocr.init_detector")
@mock.patch("mmocr.utils.ocr.tesserocr")
def test_tesseract_wrapper(mock_tesserocr, mock_init_detector, det, recog, target):
    ...
    mock_tesseract.GetComponentImages.return_value = [(None, {"x": 0, "y": 0, "w": 1, "h": 1}, 0, None)]

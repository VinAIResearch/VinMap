...
import numpy as np
import pytest
from mmocr.utils import bezier_to_polygon, is_on_same_line, sort_points, stitch_boxes_into_lines
...
def test_stitch_boxes_into_lines():
    ...
    expected_result = [
        {"box": [0, 0, 3, 0, 3, 1.5, 0, 1.5], "text": "0 1"},
        {"box": [3, 1.2, 4, 1.2, 4, 2.2, 3, 2.2], "text": "2"},
        {"box": [5, 0.5, 7, 0.5, 7, 1.75, 5, 1.75], "text": "3 4"},
    ]
...
def test_sort_points():
    points = np.array([[1, 1], [0, 0], [1, -1], [2, -2], [0, 2], [1, 1], [0, 1], [-1, 1], [-1, -1]])
    target = np.array([[-1, -1], [0, 0], [-1, 1], [0, 1], [0, 2], [1, 1], [1, 1], [2, -2], [1, -1]])
    ...
    points = [[1, 0.8], [0.8, -1], [1.8, 0.5], [1.9, -0.6], [-0.5, 2], [-1, 1.8], [-2, 0.7], [-1.6, -0.2], [-1, -0.5]]
    target = [[-1, -0.5], [-1.6, -0.2], [-2, 0.7], [-1, 1.8], [-0.5, 2], [1, 0.8], [1.8, 0.5], [1.9, -0.6], [0.8, -1]]
    assert np.allclose(target, sort_points(points))

    # concave polygon may failed
    points = [[1, 0], [-1, 0], [0, 0], [0, -1], [0.25, 1], [0.75, 1], [-0.25, 1], [-0.75, 1]]
    target = [[-1, 0], [-0.75, 1], [-0.25, 1], [0, 0], [0.25, 1], [0.75, 1], [1, 0], [0, -1]]
    with pytest.raises(AssertionError):
        assert np.allclose(target, sort_points(points))

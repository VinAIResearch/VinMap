...
import pytest
import torch
from mmocr.models.textdet.postprocess import DBPostprocessor, FCEPostprocessor, TextSnakePostprocessor
...
def test_fcenet_decode():
    ...
    fcenet_decode = FCEPostprocessor(fourier_degree=k, num_reconstr_points=50, nms_thr=0.01)
...
def test_textsnake_decode():
    ...
    maps.fill_(0.0)
    maps[:, 0:2, :, :] = -10.0
    maps[:, 0, 120:140, 20:40] = 10.0
    maps[:, 1, 120:140, 20:40] = 10.0
    maps[:, 2, 120:140, 20:40] = 0.0
    maps[:, 3, 120:140, 20:40] = 1.0
...
def test_db_decode():
    ...
    pred[0, 2:7, 2:7] = 0.8
    expect_result_quad = [[1.0, 8.0, 1.0, 1.0, 8.0, 1.0, 8.0, 8.0, 0.800000011920929]]
    expect_result_poly = [[8, 2, 8, 6, 6, 8, 2, 8, 1, 6, 1, 2, 2, 1, 6, 1, 0.800000011920929]]

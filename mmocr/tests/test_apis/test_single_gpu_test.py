...
def generate_sample_dataloader(cfg, curr_dir, img_prefix="", ann_file=""):
    ...
    loader_cfg = {**dict((k, cfg.data[k]) for k in ["workers_per_gpu", "samples_per_gpu"] if k in cfg.data)}
    test_loader_cfg = {**loader_cfg, **dict(shuffle=False, drop_last=False), **cfg.data.get("test_dataloader", {})}
...
def test_single_gpu_test_recog(cfg_file):
    ...
    data_loader = generate_sample_dataloader(cfg, curr_dir, img_prefix, ann_file)
...
def gene_sdmgr_model_dataloader(cfg, dirname, curr_dir, empty_img=False):
    json_obj = {
        ...
        "height": 348,
        "width": 348,
        "annotations": [{"box": [114.0, 19.0, 230.0, 19.0, 230.0, 1.0, 114.0, 1.0], "text": "CHOEUN", "label": 1}],
    }
    ...
    cfg.data.test = test
    cfg.model.class_list = osp.join(curr_dir, "data/kie_toy_dataset/class_list.txt")
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires cuda")
@pytest.mark.parametrize("cfg_file", ["../configs/kie/sdmgr/sdmgr_unet16_60e_wildreceipt.py"])
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires cuda")
@pytest.mark.parametrize("cfg_file", ["../configs/kie/sdmgr/sdmgr_novisual_60e_wildreceipt.py"])
def test_single_gpu_test_kie_novisual(cfg_file):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        out_dir = osp.join(tmpdirname, "tmp")
        model, data_loader = gene_sdmgr_model_dataloader(cfg, out_dir, curr_dir, empty_img=True)
        results = single_gpu_test(model, data_loader, out_dir=out_dir, is_kie=True)
        assert check_argument.is_type_list(results, dict)

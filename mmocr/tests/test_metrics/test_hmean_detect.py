...
import numpy as np
import pytest
from mmocr.core.evaluation.hmean import eval_hmean, get_gt_masks, output_ranklist


def _create_dummy_ann_infos():
    ann_infos = {
        "bboxes": np.array([[50.0, 70.0, 80.0, 100.0]], dtype=np.float32),
...
def test_get_gt_mask():
    ...
    assert np.allclose(gt_masks_ignore[0], [[120, 140, 200, 140, 200, 200, 120, 200]])


def test_eval_hmean():
    metrics = set(["hmean-iou", "hmean-ic13"])
    results = [
        {"boundary_result": [[50, 70, 80, 70, 80, 100, 50, 100, 1], [120, 140, 200, 140, 200, 200, 120, 200, 1]]}
    ]

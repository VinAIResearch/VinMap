...
def retrieve_img_tensor_and_meta(data):
    ...
    for key in must_keys:
        if key not in img_metas[0]:
            raise KeyError(f'Please add {key} to the "meta_keys" in the pipeline')
        ...
        if show or out_dir:
            if is_kie:
                ...
                if img_tensor.shape[0] != 1:
                    raise KeyError("Visualizing KIE outputs in batches is" "currently not supported.")
                gt_bboxes = data["gt_bboxes"].data[0]
                img_metas = data["img_metas"].data[0]
                must_keys = ["img_norm_cfg", "ori_filename", "img_shape"]
                for key in must_keys:
                    if key not in img_metas[0]:
                        raise KeyError(f'Please add {key} to the "meta_keys" in config.')
                # for no visual model
                if np.prod(img_tensor.shape) == 0:
                    ...
                    for img_meta in img_metas:
                        try:
                            ...
                            print(f"Load image with error: {e}, " "use empty image instead.")
                            img = np.ones(img_meta["img_shape"], dtype=np.uint8)
                        imgs.append(img)
                else:
                ...
                for j, (img, img_meta) in enumerate(zip(imgs, img_metas)):
                    ...
                    img_show = mmcv.imresize(img_show, (ori_shape[1], ori_shape[0]))
                    ...
                    model.module.show_result(
                        img_show, result[j], show=show, out_file=out_file, score_thr=show_score_thr
                    )

...
def parse_args():
    ...
    parser.add_argument("img", type=str, help="Input image file or folder path.")
    parser.add_argument("--output", type=str, default="", help="Output file/folder name for visualization")
    parser.add_argument("--det", type=str, default="PANet_IC15", help="Pretrained text detection algorithm")
    parser.add_argument(
        "--det-config",
        type=str,
        default="",
        help="Path to the custom config file of the selected det model. It " "overrides the settings in det",
    )
    parser.add_argument(
        "--det-ckpt",
        type=str,
        default="",
        help="Path to the custom checkpoint file of the selected det model. " "It overrides the settings in det",
    )
    parser.add_argument("--recog", type=str, default="SEG", help="Pretrained text recognition algorithm")
    parser.add_argument(
        "--recog-config",
        type=str,
        default="",
        help="Path to the custom config file of the selected recog model. It" "overrides the settings in recog",
    )
    parser.add_argument(
        "--recog-ckpt",
        type=str,
        default="",
        help="Path to the custom checkpoint file of the selected recog model. " "It overrides the settings in recog",
    )
    parser.add_argument("--kie", type=str, default="", help="Pretrained key information extraction algorithm")
    parser.add_argument(
        "--kie-config",
        type=str,
        default="",
        help="Path to the custom config file of the selected kie model. It" "overrides the settings in kie",
    )
    parser.add_argument(
        "--kie-ckpt",
        type=str,
        default="",
        help="Path to the custom checkpoint file of the selected kie model. " "It overrides the settings in kie",
    )
    parser.add_argument(
        "--config-dir",
        type=str,
        default=os.path.join(str(Path.cwd()), "configs/"),
        help="Path to the config directory where all the config files " 'are located. Defaults to "configs/"',
    )
    parser.add_argument("--batch-mode", action="store_true", help="Whether use batch mode for inference")
    parser.add_argument("--recog-batch-size", type=int, default=0, help="Batch size for text recognition")
    parser.add_argument("--det-batch-size", type=int, default=0, help="Batch size for text detection")
    parser.add_argument("--single-batch-size", type=int, default=0, help="Batch size for separate det/recog inference")
    parser.add_argument("--device", default=None, help="Device used for inference.")
    parser.add_argument("--export", type=str, default="", help="Folder where the results of each image are exported")
    parser.add_argument("--export-format", type=str, default="json", help="Format of the exported result file(s)")
    parser.add_argument(
        "--details", action="store_true", help="Whether include the text boxes coordinates and confidence values"
    )
    parser.add_argument("--imshow", action="store_true", help="Whether show image with OpenCV.")
    parser.add_argument("--print-result", action="store_true", help="Prints the recognised text")
    parser.add_argument("--merge", action="store_true", help="Merge neighboring boxes")
    parser.add_argument("--merge-xdist", type=float, default=20, help="The maximum x-axis distance to merge boxes")
    ...
    if args.merge and not (args.det and args.recog):
        warnings.warn(
            "Box merging will not work if the script is not" " running in detection + recognition mode.", UserWarning
        )
    if not os.path.samefile(args.config_dir, os.path.join(str(Path.cwd()))) and (
        args.det_config != "" or args.recog_config != ""
    ):
        warnings.warn("config_dir will be overridden by det-config or recog-config.", UserWarning)
    return args
...
class MMOCR:
        ...
        **kwargs,
        ...
        textdet_models = {
            ...
            "DB_r50": {
                ...
                "ckpt": "dbnet/" "dbnet_r50dcnv2_fpnc_sbn_1200e_icdar2015_20211025-9fe3b590.pth",
            },
            "DBPP_r50": {
                "config": "dbnetpp/dbnetpp_r50dcnv2_fpnc_1200e_icdar2015.py",
                "ckpt": "dbnet/" "dbnetpp_r50dcnv2_fpnc_1200e_icdar2015-20220502-d7a76fff.pth",
            },
            "DRRG": {
                "config": "drrg/drrg_r50_fpn_unet_1200e_ctw1500.py",
                "ckpt": "drrg/drrg_r50_fpn_unet_1200e_ctw1500_20211022-fb30b001.pth",
            },
            "FCE_IC15": {
                "config": "fcenet/fcenet_r50_fpn_1500e_icdar2015.py",
                "ckpt": "fcenet/fcenet_r50_fpn_1500e_icdar2015_20211022-daefb6ed.pth",
            },
            "FCE_CTW_DCNv2": {
                "config": "fcenet/fcenet_r50dcnv2_fpn_1500e_ctw1500.py",
                "ckpt": "fcenet/" + "fcenet_r50dcnv2_fpn_1500e_ctw1500_20211022-e326d7ec.pth",
            },
            "MaskRCNN_CTW": {
        ...
        textrecog_models = {
            "CRNN": {"config": "crnn/crnn_academic_dataset.py", "ckpt": "crnn/crnn_academic-a723a1c5.pth"},
            "SAR": {
                "config": "sar/sar_r31_parallel_decoder_academic.py",
                "ckpt": "sar/sar_r31_parallel_decoder_academic-dba3a4a3.pth",
            },
            "SAR_CN": {
                "config": "sar/sar_r31_parallel_decoder_chinese.py",
                "ckpt": "sar/sar_r31_parallel_decoder_chineseocr_20210507-b4be8214.pth",
            },
            "NRTR_1/16-1/8": {
                "config": "nrtr/nrtr_r31_1by16_1by8_academic.py",
                "ckpt": "nrtr/nrtr_r31_1by16_1by8_academic_20211124-f60cebf4.pth",
            },
            "NRTR_1/8-1/4": {
                "config": "nrtr/nrtr_r31_1by8_1by4_academic.py",
                "ckpt": "nrtr/nrtr_r31_1by8_1by4_academic_20211123-e1fdb322.pth",
            },
            "RobustScanner": {
                "config": "robust_scanner/robustscanner_r31_academic.py",
                "ckpt": "robustscanner/robustscanner_r31_academic-5f05874f.pth",
            },
            "SATRN": {"config": "satrn/satrn_academic.py", "ckpt": "satrn/satrn_academic_20211009-cb8b1580.pth"},
            "SATRN_sm": {"config": "satrn/satrn_small.py", "ckpt": "satrn/satrn_small_20211009-2cf13355.pth"},
            "ABINet": {"config": "abinet/abinet_academic.py", "ckpt": "abinet/abinet_academic-f718abf6.pth"},
            "ABINet_Vision": {
        ...
        kie_models = {
            "SDMGR": {
                ...
                "ckpt": "sdmgr/sdmgr_unet16_60e_wildreceipt_20220706-57c220a6.pth",
            }
        }
        ...
        if self.td and self.td not in textdet_models:
            raise ValueError(self.td, "is not a supported text detection algorthm")
            ...
            if self.kie not in kie_models:
                raise ValueError(self.kie, "is not a supported key information extraction" " algorithm")
            elif not (self.td and self.tr):
                raise NotImplementedError(
                    self.kie, "has to run together" " with text detection and recognition algorithms."
                )
        ...
        if self.td and self.td == "Tesseract":
            if tesserocr is None:
                raise ImportError(
                    "Please install tesserocr first. "
            ...
            if not det_config:
                det_config = os.path.join(config_dir, "textdet/", textdet_models[self.td]["config"])
            if not det_ckpt:
                det_ckpt = "https://download.openmmlab.com/mmocr/textdet/" + textdet_models[self.td]["ckpt"]

            self.detect_model = init_detector(det_config, det_ckpt, device=self.device)
        ...
        if self.tr and self.tr == "Tesseract":
            ...
            if not recog_config:
                recog_config = os.path.join(config_dir, "textrecog/", textrecog_models[self.tr]["config"])
            if not recog_ckpt:
                recog_ckpt = "https://download.openmmlab.com/mmocr/" + "textrecog/" + textrecog_models[self.tr]["ckpt"]

            self.recog_model = init_detector(recog_config, recog_ckpt, device=self.device)
        ...
        if self.kie:
            ...
            if not kie_config:
                kie_config = os.path.join(config_dir, "kie/", kie_models[self.kie]["config"])
            if not kie_ckpt:
                kie_ckpt = "https://download.openmmlab.com/mmocr/" + "kie/" + kie_models[self.kie]["ckpt"]

            kie_cfg = Config.fromfile(kie_config)
            self.kie_model = build_detector(kie_cfg.model, test_cfg=kie_cfg.get("test_cfg"))
    ...
    def get_tesserocr_api():
        ...
        if sys.platform == "linux":
            ...
            try:
                p = subprocess.Popen("where tesseract", stdout=subprocess.PIPE, shell=True)
    ...
    def readtext(
        ...
        **kwargs,
        ...
        if self.detect_model and self.recog_model:
            ...
            for model in list(filter(None, [self.recog_model, self.detect_model])):
                result = self.single_inference(model, args.arrays, args.batch_mode, args.single_batch_size)
    ...
    def det_recog_pp(self, result):
        final_results = []
        args = self.args
        for arr, output, export, det_recog_result in zip(args.arrays, args.output, args.export, result):
    ...
    def single_pp(self, result, model):
        for arr, output, export, res in zip(self.args.arrays, self.args.output, self.args.export, result):
            if export:
                mmcv.dump(res, export, indent=4)
            if output or self.args.imshow:
                if model == "Tesseract_det":
                    res_img = TextDetectorMixin(show_score=False).show_result(arr, res, out_file=output)
                elif model == "Tesseract_recog":
                    res_img = BaseRecognizer.show_result(arr, res, out_file=output)
                else:
    ...
    def visualize_kie_output(self, model, data, result, out_file=None, show=False):
        ...
        if img_tensor.dtype == torch.uint8:
            ...
            img = tensor2imgs(img_tensor.unsqueeze(0), **img_meta.get("img_norm_cfg", {}))[0]
        h, w, _ = img_meta.get("img_shape", img.shape)
        img_show = img[:h, :w, :]
        model.show_result(img_show, result, gt_bboxes, show=show, out_file=out_file)
    ...
    def det_recog_kie_inference(self, det_model, recog_model, kie_model=None):
        ...
        det_result = self.single_inference(det_model, self.args.arrays, self.args.batch_mode, self.args.det_batch_size)
        ...
        for filename, arr, bboxes, out_file in zip(
            self.args.filenames, self.args.arrays, bboxes_list, self.args.output
        ):
            img_e2e_res = {}
            ...
            for bbox in bboxes:
                ...
                if self.args.batch_mode:
                    ...
                    if recog_model == "Tesseract_recog":
                        recog_result = self.single_inference(recog_model, box_img, batch_mode=True)
                    else:
            ...
            if self.args.batch_mode:
                recog_results = self.single_inference(recog_model, box_imgs, True, self.args.recog_batch_size)
            ...
            if self.args.merge:
                img_e2e_res["result"] = stitch_boxes_into_lines(img_e2e_res["result"], self.args.merge_xdist, 0.5)
            ...
            if kie_model:
                ...
                for i, ann in enumerate(annotations):
                    ...
                    annotations[i]["box"] = [min_x, min_y, max_x, min_y, max_x, max_y, min_x, max_y]
                ann_info = kie_dataset._parse_anno_info(annotations)
                ann_info["ori_bboxes"] = ann_info.get("ori_bboxes", ann_info["bboxes"])
                ann_info["gt_bboxes"] = ann_info.get("gt_bboxes", ann_info["bboxes"])
                kie_result, data = model_inference(
                    kie_model, arr, ann=ann_info, return_data=True, batch_mode=self.args.batch_mode
                )
                # visualize KIE results
                self.visualize_kie_output(kie_model, data, kie_result, out_file=out_file, show=self.args.imshow)
                gt_bboxes = data["gt_bboxes"].data.numpy().tolist()
                labels = self.generate_kie_labels(kie_result, gt_bboxes, kie_model.class_list)
    ...
    def single_inference(self, model, arrays, batch_mode, batch_size=0):
        def inference(m, a, **kwargs):
            ...
            # else:
            tmp = model(return_loss=False, rescale=True, **model_inference(m, a, **kwargs, device=self.device))[0]
        ...
        for arr in tqdm(arrays):
            result.append(inference(model.cfg, arr, batch_mode=False))
            torch.cuda.empty_cache()
        return result
    ...
    def _args_processing(self, args):
        ...
        if args.output:
            ...
            if output_path.is_dir() or not output_path.exists():
                args.output = [str(output_path / f"out_{x}.png") for x in args.filenames]
            else:
                raise TypeError(f"output {output_path} cannot be an existing file!")
        else:

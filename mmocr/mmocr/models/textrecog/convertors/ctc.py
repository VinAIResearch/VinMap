...
class CTCConvertor(BaseConvertor):
    ...
    def __init__(self, dict_type="DICT90", dict_file=None, dict_list=None, with_unknown=True, lower=False, **kwargs):
    ...
    def str2tensor(self, strings):
        ...
        return {"targets": tensors, "flatten_targets": flatten_target, "target_lengths": target_lengths}
    ...
    def tensor2idx(self, output, img_metas, topk=1, return_topk=False):
        ...
        for b in range(batch_size):
            ...
            topk_value = torch.index_select(batch_topk_value[b, :, :], 0, select_idx)  # valid_seqlen * topk
            topk_idx = torch.index_select(batch_topk_idx[b, :, :], 0, select_idx)
            topk_idx_list, topk_value_list = topk_idx.numpy().tolist(), topk_value.numpy().tolist()

...
class FPNOCR(BaseModule):
    ...
    def __init__(self, in_channels, out_channels, last_stage_only=True, init_cfg=None):
        ...
        for i in range(self.num_ins):
            l_conv = ConvModule(in_channels[i], out_channels, 1, norm_cfg=dict(type="BN"))
            self.lateral_convs.append(l_conv)

        for i in range(self.num_ins - 1):
            s_conv_1x1 = ConvModule(out_channels * 2, out_channels, 1, norm_cfg=dict(type="BN"))
            s_conv_3x3 = ConvModule(out_channels, out_channels, 3, padding=1, norm_cfg=dict(type="BN"))
    ...
    def forward(self, inputs):
        ...
        """
        lateral_features = [l_conv(inputs[i]) for i, l_conv in enumerate(self.lateral_convs)]
        ...
        for i in range(len(self.smooth_convs_3x3), 0, -1):  # 3, 2, 1
            ...
            upsample_cat = torch.cat((upsample, lateral_features[i - 1]), dim=1)

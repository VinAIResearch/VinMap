...
class SequenceAttentionDecoder(BaseDecoder):
    ...
    def __init__(
        ...
        self.sequence_layer = nn.LSTM(
            input_size=dim_model, hidden_size=dim_model, num_layers=rnn_layers, batch_first=True, dropout=dropout
        )
    ...
    def forward_test(self, feat, out_enc, img_metas):
        ...
        for i in range(seq_len):
            step_out = self.forward_test_step(feat, out_enc, decode_sequence, i, img_metas)
    ...
    def forward_test_step(self, feat, out_enc, decode_sequence, current_step, img_metas):
        """
        Args:
            feat (Tensor): Tensor of shape :math:`(N, D_i, H, W)`.
            out_enc (Tensor): Encoder output of shape

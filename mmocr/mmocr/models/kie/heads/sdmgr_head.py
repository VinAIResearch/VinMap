...
class SDMGRHead(BaseModule):
    ...
    def __init__(
        ...
        train_cfg=None,
        test_cfg=None,
        init_cfg=dict(type="Normal", override=dict(name="edge_embed"), mean=0, std=0.01),
    ):
        ...
        self.rnn = nn.LSTM(
            input_size=node_input, hidden_size=hidden, num_layers=1, batch_first=True, bidirectional=bidirectional
        )
    ...
    def forward(self, relations, texts, x=None):
        ...
        all_nodes = torch.cat(
            [torch.cat([text, text.new_zeros(text.size(0), max_num - text.size(1))], -1) for text in texts]
        )
        ...
        nodes[valid] = (
            rnn_nodes[valid]
            .gather(1, (all_nums[valid] - 1).unsqueeze(-1).unsqueeze(-1).expand(-1, -1, rnn_nodes.size(-1)))
            .squeeze(1)
        )
        ...
        all_edges = torch.cat([rel.view(-1, rel.size(-1)) for rel in relations])
...
class GNNLayer(nn.Module):
    ...
    def forward(self, nodes, edges, nums):
        ...
        for num in nums:
            ...
            cat_nodes.append(
                torch.cat(
                    [sample_nodes.unsqueeze(1).expand(-1, num, -1), sample_nodes.unsqueeze(0).expand(num, -1, -1)], -1
                ).view(num**2, -1)
            )
            start += num
        ...
        for num in nums:
            residual = F.softmax(
                ...
                + coefs[start : start + num**2].view(num, num, -1),
                1,
            )

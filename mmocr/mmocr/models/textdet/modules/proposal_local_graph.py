...
class ProposalLocalGraphs:
    ...
    def generate_local_graphs(self, sorted_dist_inds, node_feats):
        ...
        for graph_ind, pivot_knn in enumerate(pivot_knns):
            ...
            knn_inds = torch.tensor([node2ind_map[i] for i in pivot_knn[1:]]).long().to(device)
            ...
            local_graph_nodes = torch.cat(
                [local_graph_nodes, torch.zeros(num_max_nodes - num_nodes, dtype=torch.long)], dim=-1
            )
    ...
    def __call__(self, preds, feat_maps):
        ...
        content_feats = content_feats.view(content_feats.shape[0], -1).to(device)

...
class OpensetKIEDataset(KIEDataset):
    ...
    def __init__(
        ...
        super().__init__(ann_file, loader, dict_file, img_prefix, pipeline, norm, False, test_mode)
        assert link_type in ["one-to-one", "one-to-many", "many-to-one", "many-to-many", "none"]
    ...
    def evaluate(self, results, metric="openset_f1", metric_options=None, **kwargs):
    ...
    @staticmethod
    def _decode_pairs_pred(nodes, labels, edges, edge_thr=0.5, link_type="one-to-one"):
        ...
        if link_type in ["none", "many-to-many"]:
            ...
            pred_pairs = [
                (n1.item(), n2.item()) if nodes[n1, 1] > nodes[n1, 2] else (n2.item(), n1.item())
                for n1, n2 in zip(*pair_inds)
                if n1 < n2
            ]
            pred_pairs = [(i, j) for i, j in pred_pairs if labels[i] == 1 and labels[j] == 2]
        else:
            links = edges.clone()
    ...
    def decode_pred(self, result):
        ...
        pairs, pairs_conf = self._decode_pairs_pred(nodes, labels, edges, self.edge_thr, self.link_type)
        pred = {
            "filename": filename,
            "boxes": boxes,
    ...
    def compute_openset_f1(self, preds, gts):
        ...
        for pred, gt in zip(preds, gts):
            ...
            img_res["edge_recall"] = 1.0 * img_res["edge_hit_num"] / max(1, len(pairs_gt))
            img_res["edge_precision"] = 1.0 * img_res["edge_hit_num"] / max(1, len(pairs_pred))
            img_res["f1"] = (
                2
                * img_res["edge_recall"]
                * img_res["edge_precision"]
                / max(1, img_res["edge_recall"] + img_res["edge_precision"])
            )
        ...
        stats = {}
        # edge f1
        total_edge_recall = 1.0 * total_edge_hit_num / max(1, total_edge_gt_num)
        total_edge_precision = 1.0 * total_edge_hit_num / max(1, total_edge_pred_num)
        edge_f1 = 2 * total_edge_recall * total_edge_precision / max(1, total_edge_recall + total_edge_precision)
        ...
        for node_idx in node_inds:
            ...
            node_res = {}
            node_res["recall"] = 1.0 * total_node_hit_num[node_idx] / max(1, total_node_gt_num[node_idx])
            node_res["precision"] = 1.0 * total_node_hit_num[node_idx] / max(1, total_node_pred_num[node_idx])
            node_res["f1"] = (
                2 * node_res["recall"] * node_res["precision"] / max(1, node_res["recall"] + node_res["precision"])
            )
            node_macro_metric[node_idx] = node_res

        node_micro_recall = 1.0 * cared_node_hit_num / max(1, cared_node_gt_num)
        node_micro_precision = 1.0 * cared_node_hit_num / max(1, cared_node_pred_num)
        node_micro_f1 = 2 * node_micro_recall * node_micro_precision / max(1, node_micro_recall + node_micro_precision)

        stats["node_openset_micro_f1"] = node_micro_f1
        stats["node_openset_macro_f1"] = np.mean([v["f1"] for k, v in node_macro_metric.items()])

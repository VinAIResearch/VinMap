...
class UniformConcatDataset(ConcatDataset):
    ...
    def __init__(
        self, datasets, separate_eval=True, show_mean_scores="auto", pipeline=None, force_apply=False, **kwargs
    ):
        new_datasets = []
        if pipeline is not None:
            assert isinstance(pipeline, list), "pipeline must be list[dict] or list[list[dict]]."
            if is_type_list(pipeline, dict):
                ...
                for sub_datasets, tmp_pipeline in zip(datasets, pipeline):
                    self._apply_pipeline(sub_datasets, tmp_pipeline, force_apply)
        ...
        if not separate_eval:
            raise NotImplementedError(
                "Evaluating datasets as a whole is not" ' supported yet. Please use "separate_eval=True"'
            )
        ...
        if show_mean_scores is True or show_mean_scores == "auto" and len(self.datasets) > 1:
            if len(set([type(ds) for ds in self.datasets])) != 1:
                raise NotImplementedError("To compute mean evaluation scores, all datasets" "must have the same type")
    ...
    def evaluate(self, results, logger=None, **kwargs):
        ...
        if self.separate_eval:
            ...
            for dataset in self.datasets:
                ...
                print_log(
                    f"\nEvaluating {dataset.ann_file} with " f"{len(results_per_dataset)} images now", logger=logger
                )

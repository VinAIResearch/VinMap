...
class OCRSegTargets:
    ...
    def __init__(
        self, label_convertor=None, attn_shrink_ratio=0.5, seg_shrink_ratio=0.25, box_type="char_rects", pad_val=255
    ):

        assert isinstance(attn_shrink_ratio, float)
        assert isinstance(seg_shrink_ratio, float)
        assert 0.0 < attn_shrink_ratio < 1.0
        assert 0.0 < seg_shrink_ratio < 1.0
    ...
    def shrink_char_quad(self, char_quad, shrink_ratio):
        ...
        """
        points = [
            [char_quad[0], char_quad[1]],
            [char_quad[2], char_quad[3]],
            [char_quad[4], char_quad[5]],
            [char_quad[6], char_quad[7]],
        ]
        shrink_points = []
        for p_idx, point in enumerate(points):
            ...
            temp_dist1 = (shrink_ratio * min_dist / dist1) if min_dist != 0 else 0.0
            temp_dist2 = (shrink_ratio * min_dist / dist2) if min_dist != 0 else 0.0

            v1 = [temp * temp_dist1 for temp in v1]
            v2 = [temp * temp_dist2 for temp in v2]

            shrink_point = [round(point[0] + v1[0] + v2[0]), round(point[1] + v1[1] + v2[1])]
    ...
    def shrink_char_rect(self, char_rect, shrink_ratio):
        ...
        poly = np.array([[x_min_s, y_min_s], [x_max_s, y_min_s], [x_max_s, y_max_s], [x_min_s, y_max_s]])

        return poly

    def generate_kernels(self, resize_shape, pad_shape, char_boxes, char_inds, shrink_ratio=0.5, binary=True):
        ...
        for i, char_box in enumerate(char_boxes):
            ...
            cv2.fillConvexPoly(char_kernel, poly.astype(np.int32), (fill_value))
    ...
    def __call__(self, results):
        ...
        binary_target = self.generate_kernels(
            resize_shape, pad_shape, char_boxes, char_inds, shrink_ratio=self.attn_shrink_ratio, binary=True
        )

        seg_target = self.generate_kernels(
            resize_shape, pad_shape, char_boxes, char_inds, shrink_ratio=self.seg_shrink_ratio, binary=False
        )

        mask = np.ones(pad_shape, dtype=np.int32)
        mask[: resize_shape[0], resize_shape[1] :] = 0

        results["gt_kernels"] = BitmapMasks([binary_target, seg_target, mask], pad_shape[0], pad_shape[1])

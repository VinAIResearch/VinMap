...
class DBNetTargets(BaseTextDetTargets):
    ...
    def __init__(self, shrink_ratio=0.4, thr_min=0.3, thr_max=0.7, min_short_size=8):
        super().__init__()
    ...
    def ignore_texts(self, results, ignore_tags):
        ...
        results["gt_masks_ignore"].masks += [
            mask for i, mask in enumerate(results["gt_masks"].masks) if ignore_tags[i]
        ]
        results["gt_masks"].masks = [mask for i, mask in enumerate(results["gt_masks"].masks) if not ignore_tags[i]]
        results["gt_labels"] = np.array([mask for i, mask in enumerate(results["gt_labels"]) if not ignore_tags[i]])
    ...
    def draw_border_map(self, polygon, canvas, mask):
        ...
        canvas[y_min_valid : y_max_valid + 1, x_min_valid : x_max_valid + 1] = np.fmax(
            1
            - distance_map[
                y_min_valid - y_min : y_max_valid - y_max + height, x_min_valid - x_min : x_max_valid - x_max + width
            ],
    ...
    def generate_targets(self, results):
        ...
        gt_shrink, ignore_tags = self.generate_kernels((h, w), polygons, self.shrink_ratio, ignore_tags=ignore_tags)

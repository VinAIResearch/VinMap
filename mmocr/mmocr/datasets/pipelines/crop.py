...
def box_jitter(points_x, points_y, jitter_ratio_x=0.5, jitter_ratio_y=0.1):
    ...
    line_list = [LineString([points[i], points[i + 1 if i < 3 else 0]]) for i in range(4)]
...
def warp_img(src_img, box, jitter_flag=False, jitter_ratio_x=0.5, jitter_ratio_y=0.1):
    ...
    if jitter_flag:
        box_jitter(points_x, points_y, jitter_ratio_x=jitter_ratio_x, jitter_ratio_y=jitter_ratio_y)

    points = [Point(points_x[i], points_y[i]) for i in range(4)]
    edges = [LineString([points[i], points[i + 1 if i < 3 else 0]]) for i in range(4)]
    ...
    pts2 = np.float32([[0, 0], [box_width, 0], [box_width, box_height], [0, box_height]])
    M = cv2.getPerspectiveTransform(pts1, pts2)
    dst_img = cv2.warpPerspective(src_img, M, (int(box_width), int(box_height)))
...
def crop_img(src_img, box, long_edge_pad_ratio=0.4, short_edge_pad_ratio=0.2):
    ...
    assert utils.is_type_list(box, (float, int))
    assert len(box) == 8
    assert 0.0 <= long_edge_pad_ratio < 1.0
    assert 0.0 <= short_edge_pad_ratio < 1.0

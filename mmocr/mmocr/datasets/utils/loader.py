...
from .backend import HardDiskAnnFileBackend, HTTPAnnFileBackend, PetrelAnnFileBackend
...
class AnnFileLoader:
    ...
    _backends = {"disk": HardDiskAnnFileBackend, "petrel": PetrelAnnFileBackend, "http": HTTPAnnFileBackend}

    def __init__(self, ann_file, parser, repeat=1, file_storage_backend="disk", file_format="txt", **kwargs):
        ...
        if file_format == "lmdb" and parser["type"] == "LineStrParser":
            raise ValueError(
                "We only support using LineJsonParser "
                "to parse lmdb file. Please use LineJsonParser "
                "in the dataset config"
            )
        self.parser = build_parser(parser)
        self.repeat = repeat
        self.ann_file_backend = self._backends[file_storage_backend](file_format, **kwargs)
...
class HardDiskLoader(AnnFileLoader):
    ...
    def __init__(self, ann_file, parser, repeat=1):
        warnings.warn("HardDiskLoader is deprecated, please use " "AnnFileLoader instead.", UserWarning)
        super().__init__(ann_file, parser, repeat, file_storage_backend="disk", file_format="txt")
...
class LmdbLoader(AnnFileLoader):
    ...
    def __init__(self, ann_file, parser, repeat=1):
        warnings.warn("LmdbLoader is deprecated, please use " "AnnFileLoader instead.", UserWarning)
        super().__init__(ann_file, parser, repeat, file_storage_backend="disk", file_format="lmdb")

...
class LmdbAnnFileBackend:
    ...
    def __init__(self, lmdb_path, encoding="utf8"):
        ...
        with env.begin(write=False) as txn:
            try:
                self.total_number = int(txn.get("num-samples".encode("utf-8")).decode(self.encoding))
                ...
                self.total_number = int(txn.get("total_number".encode("utf-8")).decode(self.encoding))
        ...
        with self.env.begin(write=False) as txn:
            if self.deprecated_format:
                ...
                if len(keys) == 4:
                    ...
                    line = json.dumps(
                        dict(filename=filename, height=height, width=width, annotations=annotations),
                        ensure_ascii=False,
                    )
                elif len(keys) == 2:
                    filename, text = keys
                    line = json.dumps(dict(filename=filename, text=text), ensure_ascii=False)
            else:
                index = index + 1
                ...
                if self.label_only:
                    ...
                    line = json.dumps(dict(filename=img_key, text=text), ensure_ascii=False)
            return line
    ...
    def _get_env(self):
        try:
            ...
            raise ImportError("Please install lmdb to enable LmdbAnnFileBackend.")
        return lmdb.open(
            self.lmdb_path,
            max_readers=1,
            readonly=True,
...
class HTTPAnnFileBackend:
    ...
    def __call__(self, ann_file):
        ...
        if self.file_format == "lmdb":
            raise NotImplementedError("Loading lmdb file on http is not supported yet.")

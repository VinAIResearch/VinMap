...
from mmocr.models.textdet.detectors.single_stage_text_detector import SingleStageTextDetector
from mmocr.models.textdet.detectors.text_detector_mixin import TextDetectorMixin
from mmocr.models.textrecog.recognizer.encode_decode_recognizer import EncodeDecodeRecognizer
...
class ONNXRuntimeDetector(TextDetectorMixin, SingleStageTextDetector):
    ...
    def simple_test(self, img: torch.Tensor, img_metas: Iterable, rescale: bool = False):
        ...
        if len(img_metas) > 1:
            boundaries = [
                self.bbox_head.get_boundary(*(onnx_pred[i].unsqueeze(0)), [img_metas[i]], rescale)
                for i in range(len(img_metas))
            ]

        else:
            boundaries = [self.bbox_head.get_boundary(*onnx_pred, img_metas, rescale)]
...
class ONNXRuntimeRecognizer(EncodeDecodeRecognizer):
    ...
    def simple_test(self, img: torch.Tensor, img_metas: Iterable, rescale: bool = False):
        ...
        label_indexes, label_scores = self.label_convertor.tensor2idx(onnx_pred, img_metas)
...
class TensorRTDetector(TextDetectorMixin, SingleStageTextDetector):
    ...
    def simple_test(self, img: torch.Tensor, img_metas: Iterable, rescale: bool = False):
        ...
        if len(img_metas) > 1:
            boundaries = [
                self.bbox_head.get_boundary(*(trt_pred[i].unsqueeze(0)), [img_metas[i]], rescale)
                for i in range(len(img_metas))
            ]

...
def eval_ocr_metric(pred_texts, gt_texts, metric="acc"):
    ...
    if "char_recall" in metric:
        char_recall = 1.0 * match_res["true_positive_char_num"] / (eps + match_res["gt_char_num"])
        eval_res["char_recall"] = char_recall

    if "char_precision" in metric:
        char_precision = 1.0 * match_res["true_positive_char_num"] / (eps + match_res["pred_char_num"])
    ...
    if "word_acc_ignore_case" in metric:
        word_acc_ignore_case = 1.0 * match_res["match_word_ignore_case"] / (eps + match_res["gt_word_num"])
        eval_res["word_acc_ignore_case"] = word_acc_ignore_case

    if "word_acc_ignore_case_symbol" in metric:
        word_acc_ignore_case_symbol = (
            1.0 * match_res["match_word_ignore_case_symbol"] / (eps + match_res["gt_word_num"])
        )

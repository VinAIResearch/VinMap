...
def save_results(img_paths, pred_labels, gt_labels, res_dir):
    ...
    lines = [f"{img} {pred} {gt}" for img, pred, gt in zip(img_paths, pred_labels, gt_labels)]
...
def main():
    ...
    parser.add_argument("--out-dir", type=str, default="./results", help="Dir to save results")
    parser.add_argument("--show", action="store_true", help="show image or save")
    ...
    for line in lines:
        ...
        kwargs_dict = {"gt_label": gt_label, "show": args.show, "out_file": "" if args.show else out_file}
    ...
    if num_gt_label == len(pred_labels):
        ...
        logger.info("\n" + "-" * 100)
        info = "eval on testset with img_root_path " f"{args.img_root_path} and img_list {args.img_list}\n"
        logger.info(info)

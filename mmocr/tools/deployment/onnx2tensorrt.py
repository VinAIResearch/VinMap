...
from mmocr.core.deployment import ONNXRuntimeDetector, ONNXRuntimeRecognizer, TensorRTDetector, TensorRTRecognizer
...
def onnx2tensorrt(
    ...
    if verify:
        mm_inputs = _prepare_input_img(input_config["input_path"], config.data.test.pipeline)
        ...
        if model_type == "recog":
            for onnx_result, trt_result in zip(onnx_out, trt_out):
                if onnx_result["text"] != trt_result["text"] or not np.allclose(
                    np.array(onnx_result["score"]), np.array(trt_result["score"]), rtol=1e-4, atol=1e-4
                ):
                    same_diff = "different"
                    break
        else:
            for onnx_result, trt_result in zip(onnx_out[0]["boundary_result"], trt_out[0]["boundary_result"]):
                if not np.allclose(np.array(onnx_result), np.array(trt_result), rtol=1e-4, atol=1e-4):
                    same_diff = "different"
                    break
        ...
        if show:
            ...
            trt_img = trt_model.show_result(
                input_config["input_path"], trt_out[0], out_file="tensorrt.jpg", show=False
            )
            if onnx_img is None:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert MMOCR models from ONNX to TensorRT")
    ...
    parser.add_argument("--trt-file", type=str, help="Path to the output TensorRT engine", default="tmp.trt")
    parser.add_argument(
        "--max-shape", type=int, nargs=4, default=[1, 3, 400, 600], help="Maximum shape of model input."
    )
    parser.add_argument(
        "--min-shape", type=int, nargs=4, default=[1, 3, 400, 600], help="Minimum shape of model input."
    )
    parser.add_argument("--workspace-size", type=int, default=1, help="Max workspace size in GiB.")
    parser.add_argument("--fp16", action="store_true", help="Enable fp16 mode")
    parser.add_argument(
        "--verify", action="store_true", help="Whether Verify the outputs of ONNXRuntime and TensorRT.", default=True
    )
    parser.add_argument(
        "--show", action="store_true", help="Whether visiualize outputs of ONNXRuntime and TensorRT.", default=True
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
if __name__ == "__main__":
    ...
    assert osp.exists(args.onnx_file), "ONNX model {} not found.".format(args.onnx_file)
    ...
    input_config = {"min_shape": args.min_shape, "max_shape": args.max_shape, "input_path": args.image_path}

...
def _prepare_data(cfg, imgs):
    ...
    if isinstance(data[0]["img"], list) and len(data) > 1:
        raise Exception("aug test does not support " f"inference with batch size " f"{len(data)}")
...
def pytorch2onnx(
    ...
    if model_type == "det":
        model.forward = partial(model.simple_test, img_metas=img_metas, rescale=True)
    else:
        model.forward = partial(model.forward, img_metas=img_metas, return_loss=False, rescale=True)
    ...
    if dynamic_export and model_type == "det":
        ...
        dynamic_axes = {"input": {0: "batch", 3: "width"}, "output": {0: "batch", 1: "seq_len", 2: "num_classes"}}
    with torch.no_grad():
        torch.onnx.export(
            model,
            (img_list[0],),
    ...
    if verify:
        ...
        if dynamic_export:
            ...
            img_list = [nn.functional.interpolate(_, scale_factor=scale_factor) for _ in img_list]
            if model_type == "det":
                img_metas[0][0]["scale_factor"] = img_metas[0][0]["scale_factor"] * (scale_factor * 2)
        ...
        if model_type == "recog":
            for onnx_result, pytorch_result in zip(onnx_out, pytorch_out):
                if onnx_result["text"] != pytorch_result["text"] or not np.allclose(
                    np.array(onnx_result["score"]), np.array(pytorch_result["score"]), rtol=1e-4, atol=1e-4
                ):
                    same_diff = "different"
                    break
        else:
            for onnx_result, pytorch_result in zip(onnx_out[0]["boundary_result"], pytorch_out[0]["boundary_result"]):
                if not np.allclose(np.array(onnx_result), np.array(pytorch_result), rtol=1e-4, atol=1e-4):
                    same_diff = "different"
                    break
...
def main():
    parser = ArgumentParser(description="Convert MMOCR models from pytorch to ONNX")
    parser.add_argument("model_config", type=str, help="Config file.")
    parser.add_argument("model_ckpt", type=str, help="Checkpint file (local or url).")
    parser.add_argument(
        "model_type", type=str, help="Detection or recognition model to deploy.", choices=["recog", "det"]
    )
    parser.add_argument("image_path", type=str, help="Input Image file.")
    parser.add_argument("--output-file", type=str, help="Output file name of the onnx model.", default="tmp.onnx")
    parser.add_argument("--device-id", default=0, help="Device used for inference.")
    parser.add_argument("--opset-version", type=int, help="ONNX opset version, default to 11.", default=11)
    parser.add_argument(
        "--verify", action="store_true", help="Whether verify the outputs of onnx and pytorch are same.", default=False
    )
    parser.add_argument("--verbose", action="store_true", help="Whether print the computation graph.", default=False)
    parser.add_argument("--show", action="store_true", help="Whether visualize final output.", default=False)
    parser.add_argument(
        "--dynamic-export", action="store_true", help="Whether dynamically export onnx model.", default=False
    )

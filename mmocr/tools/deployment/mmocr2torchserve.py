...
def parse_args():
    parser = ArgumentParser(description="Convert MMOCR models to TorchServe `.mar` format.")
    parser.add_argument("config", type=str, help="config file path")
    parser.add_argument("checkpoint", type=str, help="checkpoint file path")
    parser.add_argument(
        "--output-folder", type=str, required=True, help="Folder where `{model_name}.mar` will be created."
    )
    parser.add_argument(
        "--model-name",
        type=str,
...
if __name__ == "__main__":
    ...
    mmocr2torchserve(args.config, args.checkpoint, args.output_folder, args.model_name, args.model_version, args.force)

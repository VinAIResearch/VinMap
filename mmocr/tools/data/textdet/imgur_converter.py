...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate training, validation and test set of IMGUR ")
...
def collect_imgur_info(root_path, annotation_filename, print_every=1000):
    ...
    for i, img_name in enumerate(images):
        ...
        img_info = dict(file_name=img_name + ".jpg", height=img.shape[0], width=img.shape[1])
        ...
        for ann_id in annotation["index_to_ann_map"][img_name]:
            ...
            box = np.fromstring(ann["bounding_box"][1:-2], sep=",", dtype=float)
...
def main():
    ...
    for split in ["train", "val", "test"]:
        ...
        with mmcv.Timer(print_tmpl="It takes {}s to convert IMGUR annotation"):
            anno_infos = collect_imgur_info(root_path, f"imgur5k_annotations_{split}.json")
            convert_annotations(anno_infos, osp.join(root_path, f"instances_{split}.json"))

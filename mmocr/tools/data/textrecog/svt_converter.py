...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate testset of svt by cropping box image.")
    parser.add_argument(
        "root_path", help="Root dir path of svt, where test.xml in," 'for example, "data/mixture/svt/svt1/"'
    )
    parser.add_argument("--resize", action="store_true", help="Whether resize cropped image to certain size.")
...
def main():
    ...
    if not osp.exists(src_label_file):
        raise Exception(f"{src_label_file} not exists, please check and try again.")
    ...
    for image_node in root.findall("image"):
        ...
        for rectangle in image_node.find("taggedRectangles"):
            ...
            lines.append(f"{osp.basename(dst_image_root)}/{dst_img_name} " f"{text_label} {lex_size} {lexicon}")
    list_to_file(dst_label_file, lines)
    print(f"Finish to generate svt testset, " f"with label file {dst_label_file}")

...
def convert_art(root_path, split, ratio, format):
    ...
    annotation_path = osp.join(root_path, "annotations/train_task2_labels.json")
    ...
    for prefix in img_prefixes:
        ...
        if format == "txt":
            ...
            labels.append(json.dumps({"filename": f"crops/{dst_img_name}", "text": text_label}, ensure_ascii=False))
...
def main():
    ...
    convert_art(root_path=root_path, split="train", ratio=args.val_ratio, format=args.format)
    if args.val_ratio > 0:
        print("Processing validation set...")
        convert_art(root_path=root_path, split="val", ratio=args.val_ratio, format=args.format)
    print("Finish")

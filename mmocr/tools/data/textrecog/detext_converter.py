...
def generate_ann(root_path, split, image_infos, preserve_vertical, format):
    ...
    for image_info in image_infos:
        index = 1
        src_img_path = osp.join(root_path, "imgs", split, image_info["file_name"])
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate training and val set of DeText ")
...
def main():
    ...
    for split in ["training", "val"]:
        ...
        with mmcv.Timer(print_tmpl="It takes {}s to convert DeText annotation"):
            files = collect_files(osp.join(root_path, "imgs", split), osp.join(root_path, "annotations", split))
            image_infos = collect_annotations(files, nproc=args.nproc)
            generate_ann(root_path, split, image_infos, args.preserve_vertical, args.format)

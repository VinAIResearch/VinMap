...
def convert_annotations(root_path, split, format):
    ...
    lines = []
    with open(osp.join(root_path, "annotations", f"Challenge1_{split}_Task3_GT.txt"), "r", encoding='"utf-8-sig') as f:
        annos = f.readlines()
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate training and test set of IC11")

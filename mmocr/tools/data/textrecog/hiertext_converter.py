...
def process_level(
    ...
    if format == "txt":
        label = f"{osp.basename(dst_image_root)}/{dst_img_name}" f" {text_label}"
    elif format == "jsonl":
        label = json.dumps({"filename": f"{osp.basename(dst_image_root)}/{dst_img_name}", "text": text_label})
    else:
        raise NotImplementedError
    return label


def process_img(args, src_image_root, dst_image_root, ignore_image_root, level, preserve_vertical, split, format):
    # Dirty hack for multi-processing
    img_idx, img_annos = args
    src_img = mmcv.imread(osp.join(src_image_root, img_annos["image_id"] + ".jpg"))
    labels = []
...
def parse_args():
    ...
    parser.add_argument("--level", default="word", help="Crop word or line level instance", choices=["word", "line"])
    parser.add_argument(

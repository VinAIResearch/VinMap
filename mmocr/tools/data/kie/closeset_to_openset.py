    ...
    for anno in closeset_obj["annotations"]:
        ...
        if label == ignore_idx:
            ...
            if edge is not None:
                ...
                tmp_key = "key"
                if label in closeset_key_inds:
                    label_with_same_edge = closeset_value_inds[closeset_key_inds.index(label)]
                elif label in closeset_value_inds:
                    label_with_same_edge = closeset_key_inds[closeset_value_inds.index(label)]
                    tmp_key = "value"
                edge_counterpart = label_to_edge.get(label_with_same_edge, None)
...
def process(closeset_file, openset_file, merge_bg_others=False, n_proc=10):
    ...
    openset_lines = mmcv.track_parallel_progress(convert_func, closeset_lines, nproc=n_proc)
...
def parse_args():
    ...
    parser.add_argument(
        "--merge",
        action="store_true",
        help='Merge two classes: "background" and "others" in closeset ' "to one class in openset.",
    )
    parser.add_argument("--n_proc", type=int, default=10, help="Number of process.")

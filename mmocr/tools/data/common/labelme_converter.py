...
def parse_labelme_json(json_file, img_dir, out_dir, tasks, ignore_marker="###", recog_format="jsonl", warp_flag=False):
    invalid_res = [[], [], []]
    ...
    for box_info in shape_info:
        ...
        if shape == "rectangle":
            warp_flag = False
            quad = [poly[0], poly[1], poly[2], poly[1], poly[2], poly[3], poly[0], poly[3]]
        else:
        ...
        if "recog" in tasks:
            ...
            if recog_format == "txt":
                recog_crop_line_str_list.append(f"{img_path_cropped_img} {text_label}")
            elif recog_format == "jsonl":
                recog_crop_line_str_list.append(json.dumps({"filename": img_path_cropped_img, "text": text_label}))
            else:
            ...
            if warp_flag:
                ...
                if recog_format == "txt":
                    recog_warp_line_str_list.append(f"{img_path_warpped_img} {text_label}")
                elif recog_format == "jsonl":
                    recog_warp_line_str_list.append(json.dumps({"filename": img_path_warpped_img, "text": text_label}))
        idx += 1

    line_json = {"file_name": img_file, "height": img_height, "width": img_width, "annotations": annos}
    det_line_json_list.append(json.dumps(line_json, ensure_ascii=False))

    return [det_line_json_list, recog_crop_line_str_list, recog_warp_line_str_list]


def process(json_dir, img_dir, out_dir, tasks=["det"], nproc=1, recog_format="jsonl", warp=False):
    mmcv.mkdir_or_exist(out_dir)

    json_file_list = glob.glob(osp.join(json_dir, "*.json"))

    parse_labelme_json_func = partial(
        parse_labelme_json, img_dir=img_dir, out_dir=out_dir, tasks=tasks, recog_format=recog_format, warp_flag=warp
    )

    if nproc <= 1:
        total_results = mmcv.track_progress(parse_labelme_json_func, json_file_list)
    else:
        total_results = mmcv.track_parallel_progress(
            parse_labelme_json_func, json_file_list, keep_order=True, nproc=nproc
        )
    ...
    if "recog" in tasks:
        ...
        if warp:
            recog_out_file_warp = osp.join(out_dir, f"warp_train_label.{recog_format}")
...
def parse_args():
    ...
    parser.add_argument("out_dir", help="Dir to save annotations in mmocr format.")
    parser.add_argument("--tasks", nargs="+", help='Tasks to be processed, can be only "det" or both: "det recog"')
    parser.add_argument("--nproc", type=int, default=1, help="Number of process.")
    parser.add_argument(
        "--format",
        default="jsonl",
    ...
    parser.add_argument("--warp", help="Store warped img for recognition task", action="store_true")
...
def main():
    ...
    process(args.json_dir, args.image_dir, args.out_dir, args.tasks, args.nproc, args.format, args.warp)

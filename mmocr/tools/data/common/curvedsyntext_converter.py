...
def modify_annotation(ann, num_sample, start_img_id=0, start_ann_id=0):
    ...
    ann["segmentation"] = np.asarray(sort_points(polygon_pts)).reshape(1, -1).tolist()
...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert CurvedSynText150k to COCO format")
    parser.add_argument("root_path", help="CurvedSynText150k  root path")
    parser.add_argument("-o", "--out-dir", help="Output path")
    parser.add_argument(
        "-n", "--num-sample", type=int, default=4, help="Number of sample points at each Bezier curve."
    )
...
def convert_annotations(data, path_prefix, num_sample, nproc, start_img_id=0, start_ann_id=0):
    modify_image_info_with_params = partial(modify_image_info, path_prefix=path_prefix, start_img_id=start_img_id)
    modify_annotation_with_params = partial(
        modify_annotation, num_sample=num_sample, start_img_id=start_img_id, start_ann_id=start_ann_id
    )
    if nproc > 1:
        data["annotations"] = mmcv.track_parallel_progress(
            modify_annotation_with_params, data["annotations"], nproc=nproc
        )
        data["images"] = mmcv.track_parallel_progress(modify_image_info_with_params, data["images"], nproc=nproc)
    else:
        data["annotations"] = mmcv.track_progress(modify_annotation_with_params, data["annotations"])
...
def main():
    ...
    data2 = convert_annotations(
        anns, "emcs_imgs", args.num_sample, args.nproc, start_img_id=start_img_id, start_ann_id=start_ann_id
    )

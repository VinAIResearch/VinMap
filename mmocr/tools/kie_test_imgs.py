...
def save_results(model, img_meta, gt_bboxes, result, out_dir):
    assert "filename" in img_meta, 'Please add "filename" ' 'to "meta_keys" in config.'
    assert "ori_texts" in img_meta, 'Please add "ori_texts" ' 'to "meta_keys" in config.'

    out_json_file = osp.join(out_dir, osp.basename(img_meta["filename"]) + ".json")
    ...
    json_result = [
        {
            "text": text,
            "box": box,
            "pred": idx_to_cls.get(pred.argmax(-1).cpu().item(), pred.argmax(-1).cpu().item()),
            "conf": pred.max(-1)[0].cpu().item(),
        }
        for text, box, pred in zip(img_meta["ori_texts"], gt_bboxes, result["nodes"])
    ]
    ...
    for i, data in enumerate(data_loader):
        ...
        if show or out_dir:
            ...
            for i, (img, img_meta) in enumerate(zip(imgs, img_metas)):
                ...
                if out_dir:
                    out_file = osp.join(out_dir, osp.basename(img_meta["filename"]))
                else:
                    out_file = None
                ...
                if out_dir:
                    save_results(model, img_meta, gt_bboxes[i], result[i], out_dir)
...
def parse_args():
    parser = argparse.ArgumentParser(description="MMOCR visualize for kie model.")
    parser.add_argument("config", help="Test config file path.")
    parser.add_argument("checkpoint", help="Checkpoint file.")
    parser.add_argument("--show", action="store_true", help="Show results.")
    parser.add_argument("--out-dir", help="Directory where the output images and results will be saved.")
    parser.add_argument("--local_rank", type=int, default=0)
    parser.add_argument("--device", help="Use int or int list for gpu. Default is cpu", default=None)

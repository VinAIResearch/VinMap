...
def collect_files(img_dir, gt_dir):
    ...
    for img_file in imgs_list:
        gt_file = gt_dir + "/" + osp.splitext(osp.basename(img_file))[0] + ".txt"
...
def load_img_info(files, dataset):
    ...
    for line in gt_list:
        ...
        if (dataset == "icdar2015" and strs[8] == "###") or (dataset == "icdar2017" and strs[9] == "###"):
            iscrowd = 1
            print("ignore text")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert Icdar2015 or Icdar2017 annotations to COCO format")
    parser.add_argument("-icdar_path", "--image_path", help="image root path")
    parser.add_argument("-gt_path", "--gt_path", help="gt root path")
    parser.add_argument("-o", "--out-dir", help="output path")
    parser.add_argument("-d", "--dataset", required=True, help="icdar2017 or icdar2015")
...
def main():
    ...
    for split, json_name in set_name.items():
        ...
        with mmcv.Timer(print_tmpl="It takes {}s to convert icdar annotation"):
            ...
            image_infos = collect_annotations(files, args.dataset, nproc=args.nproc)

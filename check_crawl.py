...
class ModelWrapper():
    ...
    def e2e_infer(self, data_checks, folder_path, result_file):
        '''
        e2e Inferences
        ...
        with open(data_checks, 'r', encoding="utf-8") as f:
        ...
        for filenamei in data_checks['images']:
            filename = filenamei['path_file']
            readtext(folder_path+'/'+'fig', filename)
            torch.cuda.empty_cache()
            obj = {
              'file_path': filename,
              'link': filenamei['link'],
              'trang': filenamei['trang'],
              'link_bai_viet': filenamei['link_bai_viet'],
              'result': ''
            }
            results = 0
            ...
            for f in (os.listdir(image_folder_path)):
                ...
                if result ['pred_label'] == 1: # Not VietNam
                    results = 1
                    obj['result'] = 'This is not a map.. SKIPPED'
                    continue
                ...
                if result ['pred_label'] == 0: # Not VietNam
                    results = 1
                    obj['result'] = 'This is not VN map.. SKIPPED'
                    continue
                ff = None
            ...
            #     (VN map and contain (Truong Sa or Hoang Sa))
            if results == 0:
                obj['result'] = 'This is VN map. It does not contain (Truong Sa and Hoang Sa).. ALERT' 
            elif (results == 1):
                obj['result'] = 'This is not VN map.. SKIPPED'
            elif (results == 2):
                obj['result'] = 'This is VN map, It contains (Truong Sa or Hoang Sa).. OK'
...
if __name__ == "__main__":
  ...
  while True:
    ...
    result_file = f"result_{formatted_date}.json"
    result_file_path = f'/root/mic-crawl-vin/data-crawl/{formatted_date}/{result_file}'
    data_checks = f'/root/mic-crawl-vin/data-crawl/{formatted_date}/data.json'
    if not os.path.exists(result_file_path) and os.path.exists(data_checks):
      try:
        with open(data_checks, "r", encoding="utf-8") as f:
          data = json.load(f)
          f.close()
        if data['crawl_status'] == 'done':
          print("Start check")
          wrapper = ModelWrapper(args)
          wrapper.e2e_infer(data_checks, args.infer_path, result_file_path)
        else:
          print("CRAWLING!!!!!!!!!")
          time.sleep(60)
